...
;(assert 
;	(forall( (|S_$86| $state) (|p_$89| $ptr) (|q_$9| $ptr) (|l_$0| $label))
;		(!(= 
;			($in_domain_lab |S_$86| |p_$89| |q_$9| |l_$0|)
;			($in_domain |S_$86| |p_$89| |q_$9|)
;		)  
;		:weight  0 
;		:pattern ( ($in_domain_lab |S_$86| |p_$89| |q_$9| |l_$0|)))))

(assert 
	(forall( (s $state) (p1 $ptr) (p2 $ptr) (l $label))
		(!
			(and
				(=> ($in_domain s p1 p2) ($in_domain_lab s p1 p2 l))
				(=> ($in_domain_lab s p1 p2 l) ($in_domain s p1 p2))
			)
		 
		:weight 0 
		:pattern ( ($in_domain_lab s p1 p2 l)))))
		
(assert (forall ((S@@30 $state) (p@@34 $ptr) (q@@7 $ptr) (l@@0 $label) ) (!  (and (=> ($in_domain_lab S@@30 p@@34 q@@7 l@@0) ($in_domain S@@30 p@@34 q@@7)) (=> ($in_domain S@@30 p@@34 q@@7) ($in_domain_lab S@@30 p@@34 q@@7 l@@0)))
 :qid |vcc2comp.980:17|
 :weight 0
 :skolemid |108|
...
(assert (forall( (|S_$85| $state) (|p_$88| $ptr) (|q_$8| $ptr) (|l| $label))
	(!(=> ($in_domain_lab |S_$85| |p_$88| |q_$8| |l|) ($inv_lab |S_$85| |p_$88| |l|))  :weight  0 :pattern ( ($in_domain_lab |S_$85| |p_$88| |q_$8| |l|)))))
   ;Program
(assert (not
(let ((anon12_assertion (and
	;($in_domain $s ($ptr ^List P@l) ($ptr ^List P@l)); ($in_domain_lab $s ($ptr ^List P@l) ($ptr ^List P@l) l@public))
    ;($in_domain $s ($ptr ^List P@l) ($ptr ^List P@l))
...
(let ((anon00_LoopBody (=> (and true
    ...
    true ;(and anon01_Else anon01_Then)
	anon01_Then
)))
...
(let ((anon12@5 (=> (and true
    ...
    true ;anon12@5_assertion
    anon12@6
))))
(let ((anon12@4 (=> (and true
    (=> (not (= ($ref ($ptr ^Node L@n@0)) ($ref $null))) ($set_in ($ptr ^Node L@n@0) ($owns $s ($ptr ^List P@l))))
  )(and 
    true ;anon12@4_assertion
    anon12@5
))))
