...
implementation sum_max(P#a: int, P#N: int, P#maxout: int) returns ($result: int, OP#S: $map_t..^^u4.^^u4)
{
  ...
  anon7:
    ...
    loopState#0 := $s;
    while (true)
      invariant (forall Q#k$1^20.31#tc1: int :: $in_range_u4(Q#k$1^20.31#tc1) ==> Q#k$1^20.31#tc1 < L#i ==> $select.$map_t..^^u4.^^u4(OP#S, Q#k$1^20.31#tc1 + 1) == $select.$map_t..^^u4.^^u4(OP#S, Q#k$1^20.31#tc1) + $mem($s, $idx($ptr(^^u4, P#a), Q#k$1^20.31#tc1, ^^u4)));
      invariant L#sum == $select.$map_t..^^u4.^^u4(OP#S, L#i);
      invariant (forall Q#k$1^22.31#tc1: int :: $in_range_u4(Q#k$1^22.31#tc1) ==> Q#k$1^22.31#tc1 < L#i ==> $mem($s, $idx($ptr(^^u4, P#a), Q#k$1^22.31#tc1, ^^u4)) <= L#max);
      ...
      invariant L#i <= P#N;
    {
      anon6:
        ...
        assume $local_value_is($s, #tok$1^19.3, #loc.maxout, $ptr_to_int($ptr(^^u4, P#maxout)), $ptr_to(^^u4)) && $local_value_is_ptr($s, #tok$1^19.3, #loc.maxout, $ptr(^^u4, P#maxout), $ptr_to(^^u4));
        assume $local_value_is($s, #tok$1^19.3, #loc.N, P#N, ^^u4);
        assume $local_value_is($s, #tok$1^19.3, #loc.a, $ptr_to_int($ptr(^^u4, P#a)), $ptr_to(^^u4)) && $local_value_is_ptr($s, #tok$1^19.3, #loc.a, $ptr(^^u4, P#a), $ptr_to(^^u4));
        ...
        if (L#i < P#N)
        {
          anon3:
            ...
            assert (forall Q#x$1^26.28#tc2: int :: L#max <= Q#x$1^26.28#tc2 ==> L#sum <= $op_mul(L#i, Q#x$1^26.28#tc2));
            // assume forall(mathint x; true; ==>(<=(max, x), <=(sum, *(i, x)))); 
            assume (forall Q#x$1^26.28#tc2: int :: L#max <= Q#x$1^26.28#tc2 ==> L#sum <= $op_mul(L#i, Q#x$1^26.28#tc2));
          ...
          anon4:
            ...
            assume L#sum + $mem($s, $idx($ptr(^^u4, P#a), L#i, ^^u4)) < 4294967295;
            // assert <=(sum, *(i, max)); 
