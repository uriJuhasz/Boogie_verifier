...
axiom $is_composite(^_toto);

axiom $ptr_level(^_toto) == 0;

axiom $sizeof(^_toto) == 8;

axiom (forall #s1: $state, #s2: $state, #p: $ptr :: { $inv2(#s1, #s2, #p, ^_toto) } $inv2(#s1, #s2, #p, ^_toto) == ($typed(#s2, #p) && $set_eq($owns(#s2, #p), $set_empty())));

axiom (forall #s2: $state, #r: int :: { $inv_lab(#s2, $ptr(^_toto, #r), l#public) } $inv_lab(#s2, $ptr(^_toto, #r), l#public) == ($typed(#s2, $ptr(^_toto, #r)) && $set_eq($owns(#s2, $ptr(^_toto, #r)), $set_empty())));

axiom (forall #q: $ptr, #r: int :: { $in_full_extent_of(#q, $ptr(^_toto, #r)) } $in_full_extent_of(#q, $ptr(^_toto, #r)) == (#q == $ptr(^_toto, #r) || #q == $dot($ptr(^_toto, #r), _toto.$owns) || #q == $dot($ptr(^_toto, #r), _toto.inti)));

axiom (forall #s: $state, #q: $ptr, #r: int :: { $in_extent_of(#s, #q, $ptr(^_toto, #r)) } $in_extent_of(#s, #q, $ptr(^_toto, #r)) == $in_struct_extent_of(#q, $ptr(^_toto, #r))) && (forall #s: $state, #q: $ptr, #r: int :: { $in_extent_of(#s, #q, $ptr(^_toto, #r)) } $typed(#s, $ptr(^_toto, #r)) ==> $in_extent_of(#s, #q, $ptr(^_toto, #r)) == (#q == $ptr(^_toto, #r) || $emb(#s, #q) == $ptr(^_toto, #r)));

axiom (forall #q: $ptr, #r: int :: { $in_span_of(#q, $ptr(^_toto, #r)) } $in_span_of(#q, $ptr(^_toto, #r)) == (#q == $ptr(^_toto, #r) || #q == $dot($ptr(^_toto, #r), _toto.$owns) || #q == $dot($ptr(^_toto, #r), _toto.inti)));

axiom (forall #p: $ptr, #s1: $state, #s2: $state :: { $state_spans_the_same(#s1, #s2, #p, ^_toto) } $state_spans_the_same(#s1, #s2, #p, ^_toto) == ($mem_eq(#s1, #s2, $dot(#p, _toto.$owns)) && $mem_eq(#s1, #s2, $dot(#p, _toto.inti))));

axiom (forall #p: $ptr, #s1: $state, #s2: $state :: { $state_nonvolatile_spans_the_same(#s1, #s2, #p, ^_toto) } $state_nonvolatile_spans_the_same(#s1, #s2, #p, ^_toto) == ($mem_eq(#s1, #s2, $dot(#p, _toto.$owns)) && $mem_eq(#s1, #s2, $dot(#p, _toto.inti))));

axiom $is_claimable(^_toto) == false;

axiom $has_volatile_owns_set(^_toto) == false;

axiom (forall #s1: $state, #r: int :: { $extent_mutable(#s1, $ptr(^_toto, #r)) } $extent_mutable(#s1, $ptr(^_toto, #r)) == $mutable(#s1, $ptr(^_toto, #r)));

axiom (forall #s1: $state, #s2: $state, #r: int :: { $extent_is_fresh(#s1, #s2, $ptr(^_toto, #r)) } $extent_is_fresh(#s1, #s2, $ptr(^_toto, #r)) == $is_fresh(#s1, #s2, $ptr(^_toto, #r)));

axiom (forall #s1: $state, #r: int :: { $extent_zero(#s1, $ptr(^_toto, #r)) } $extent_zero(#s1, $ptr(^_toto, #r)) == ($mem(#s1, $dot($ptr(^_toto, #r), _toto.inti)) == 0));

const unique _toto.$owns: $field;

axiom $static_field_properties(_toto.$owns, ^_toto);

axiom $is_primitive_non_volatile_field(_toto.$owns);

axiom (forall #p: $ptr :: { $dot(#p, _toto.$owns) } $is(#p, ^_toto) ==> $dot(#p, _toto.$owns) == $ptr(^$#ptrset, $ghost_ref(#p, _toto.$owns)) && $extent_hint($dot(#p, _toto.$owns), #p));

axiom (forall #p: $ptr, #s: $state :: { $ts(#s, $dot(#p, _toto.$owns)) } { $st(#s, $dot(#p, _toto.$owns)) } $typed2(#s, #p, ^_toto) ==> $field_properties(#s, #p, _toto.$owns, ^$#ptrset, false));

axiom $owns_set_field(^_toto) == _toto.$owns;

const unique _toto.inti: $field;

axiom $static_field_properties(_toto.inti, ^_toto);

axiom $is_primitive_non_volatile_field(_toto.inti);

axiom $field_offset(_toto.inti) == 0;

axiom (forall #p: $ptr :: { $dot(#p, _toto.inti) } $is(#p, ^_toto) ==> $dot(#p, _toto.inti) == $ptr(^^u8, $ref(#p) + 0) && $extent_hint($dot(#p, _toto.inti), #p));

axiom (forall #p: $ptr, #s: $state :: { $ts(#s, $dot(#p, _toto.inti)) } { $st(#s, $dot(#p, _toto.inti)) } $typed2(#s, #p, ^_toto) ==> $field_properties(#s, #p, _toto.inti, ^^u8, false));
...
implementation Add64(P#x: int, P#n: int) returns ($result: int)
{
  ...
  anon1:
    ...
    assume (forall #p: $ptr :: { $in_writes_at(#wrTime$1^85.1, #p) } $in_writes_at(#wrTime$1^85.1, #p) == false);
...
implementation InSet64(P#x: int, P#n: int) returns ($result: bool)
{
  ...
  anon3:
    ...
    assume (forall #p: $ptr :: { $in_writes_at(#wrTime$1^92.1, #p) } $in_writes_at(#wrTime$1^92.1, #p) == false);
    // assume @in_range_u8(x); 
    assume $in_range_u8(P#x);
    // assume @in_range_u8(n); 
    assume $in_range_u8(P#n);
...
implementation Toggle64(P#x: int, P#n: int) returns ($result: int)
{
  ...
  anon5:
    ...
    assume (forall #p: $ptr :: { $in_writes_at(#wrTime$1^99.1, #p) } $in_writes_at(#wrTime$1^99.1, #p) == false);
    // assume @in_range_u8(x); 
    assume $in_range_u8(P#x);
    // assume @in_range_u8(n); 
    assume $in_range_u8(P#n);
...
implementation Add32(P#x: int, P#n: int) returns ($result: int)
{
  ...
  anon7:
    ...
    assume (forall #p: $ptr :: { $in_writes_at(#wrTime$1^106.1, #p) } $in_writes_at(#wrTime$1^106.1, #p) == false);
...
implementation InSet32(P#x: int, P#n: int) returns ($result: bool)
{
  ...
  anon9:
    ...
    assume (forall #p: $ptr :: { $in_writes_at(#wrTime$1^113.1, #p) } $in_writes_at(#wrTime$1^113.1, #p) == false);
    // assume @in_range_u4(x); 
    assume $in_range_u4(P#x);
    // assume @in_range_u8(n); 
    assume $in_range_u8(P#n);
...
implementation Toggle32(P#x: int, P#n: int) returns ($result: int)
{
  ...
  anon11:
    ...
    assume (forall #p: $ptr :: { $in_writes_at(#wrTime$1^120.1, #p) } $in_writes_at(#wrTime$1^120.1, #p) == false);
    // assume @in_range_u4(x); 
    assume $in_range_u4(P#x);
    // assume @in_range_u8(n); 
    assume $in_range_u8(P#n);
...
implementation SpecialValues64()
{
  ...
  anon13:
    ...
    assume (forall #p: $ptr :: { $in_writes_at(#wrTime$1^127.1, #p) } $in_writes_at(#wrTime$1^127.1, #p) == false);
    // uint64_t all; 
    ...
    assume (forall Q#i$1^129.26#tc1: int :: $in_range_u8(Q#i$1^129.26#tc1) ==> Q#i$1^129.26#tc1 < 64 ==> $select.$map_t..^^u8.^^bool(F#ToBm64(L#all), Q#i$1^129.26#tc1));
    // assume forall(uint64_t i; @in_range_u8(i); ==>(<(i, 64), unchecked!(@map_get(ToBm64(zero), i)))); 
    assume (forall Q#i$1^130.26#tc1: int :: $in_range_u8(Q#i$1^130.26#tc1) ==> Q#i$1^130.26#tc1 < 64 ==> !$select.$map_t..^^u8.^^bool(F#ToBm64(L#zero), Q#i$1^130.26#tc1));
...
implementation SpecialValues32()
{
  ...
  anon14:
    ...
    assume (forall #p: $ptr :: { $in_writes_at(#wrTime$1^137.1, #p) } $in_writes_at(#wrTime$1^137.1, #p) == false);
    // uint32_t all; 
    ...
    assume (forall Q#i$1^139.26#tc1: int :: $in_range_u8(Q#i$1^139.26#tc1) ==> Q#i$1^139.26#tc1 < 32 ==> $select.$map_t..^^u8.^^bool(F#ToBm32(L#all), Q#i$1^139.26#tc1));
    // assume forall(uint64_t i; @in_range_u8(i); ==>(<(i, 32), unchecked!(@map_get(ToBm32(zero), i)))); 
    assume (forall Q#i$1^140.26#tc1: int :: $in_range_u8(Q#i$1^140.26#tc1) ==> Q#i$1^140.26#tc1 < 32 ==> !$select.$map_t..^^u8.^^bool(F#ToBm32(L#zero), Q#i$1^140.26#tc1));
...
implementation FortyTwo64()
{
  ...
  anon15:
    ...
    assume (forall #p: $ptr :: { $in_writes_at(#wrTime$1^147.1, #p) } $in_writes_at(#wrTime$1^147.1, #p) == false);
    // uint64_t ft; 
...
implementation FortyTwo32()
{
  ...
  anon16:
    ...
    assume (forall #p: $ptr :: { $in_writes_at(#wrTime$1^159.1, #p) } $in_writes_at(#wrTime$1^159.1, #p) == false);
    // uint32_t ft; 
...
procedure test(P#ob: int) returns ($result: bool);
  requires $wrapped($s, $ptr(^_toto, P#ob), ^_toto);
  requires $mem($s, $dot($ptr(^_toto, P#ob), _toto.inti)) == 0;
  modifies $s, $cev_pc;
  free ensures (forall #p: $ptr :: { $mem($s, #p) } $irrelevant(old($s), #p) || #p == $ptr(^_toto, P#ob) || $mem_eq(old($s), $s, #p)) && (forall #p: $ptr :: { $st($s, #p) } $irrelevant(old($s), #p) || #p == $ptr(^_toto, P#ob) || $st_eq(old($s), $s, #p)) && (forall #p: $ptr :: { $ts($s, #p) } $irrelevant(old($s), #p) || #p == $ptr(^_toto, P#ob) || $ts_eq(old($s), $s, #p)) && $timestamp_post(old($s), $s);
  free ensures $call_transition(old($s), $s);



implementation test(P#ob: int) returns ($result: bool)
{
  ...
  anon17:
    ...
    assume $can_use_all_frame_axioms($s);
    assume $local_value_is($s, #tok$1^175.1, #loc.ob, $ptr_to_int($ptr(^_toto, P#ob)), $ptr_to(^_toto)) && $local_value_is_ptr($s, #tok$1^175.1, #loc.ob, $ptr(^_toto, P#ob), $ptr_to(^_toto));
    assume #wrTime$1^175.1 == $current_timestamp($s);
    assume (forall #p: $ptr :: { $in_writes_at(#wrTime$1^175.1, #p) } $in_writes_at(#wrTime$1^175.1, #p) == (#p == $ptr(^_toto, P#ob)));
    assume $thread_owned($s, $ptr(^_toto, P#ob));
    // assume @in_range_phys_ptr((mathint)ob); 
    assume $in_range_phys_ptr($ref($ptr(^_toto, P#ob)));
    // assert @_vcc_in_domain(@state, ob, ob); 
    assert $in_domain_lab($s, $ptr(^_toto, P#ob), $ptr(^_toto, P#ob), l#public);
    ...
    assert $wrapped($s, $ptr(^_toto, P#ob), ^_toto);
    // assert @writes_check(ob); 
    assert $top_writable($s, #wrTime$1^175.1, $ptr(^_toto, P#ob));
    // assume @_vcc_inv(@state, ob); 
    assume $inv($s, $ptr(^_toto, P#ob), ^_toto);
    // assume ==(owns#10, @_vcc_owns(@state, ob)); 
    assume owns#10 == $owns($s, $ptr(^_toto, P#ob));
    // assume @_vcc_pre_static_unwrap(@state); 
    assume $pre_static_unwrap($s);
    // @_vcc_static_unwrap(pure(ob), prestate#11)
    call $static_unwrap($ptr(^_toto, P#ob), prestate#11);
    assume $good_state_ext(#tok$1^181.19, $s);
    // assume @_vcc_full_stop(@state); 
    assume $full_stop($s);
    // assert ==(*((ob->inti)), 0); 
    assert $mem($s, $dot($ptr(^_toto, P#ob), _toto.inti)) == 0;
    // assume ==(*((ob->inti)), 0); 
    assume $mem($s, $dot($ptr(^_toto, P#ob), _toto.inti)) == 0;
    // assert @_vcc_typed2(@state, (ob->inti)); 
    assert $typed2($s, $dot($ptr(^_toto, P#ob), _toto.inti), ^^u8);
    // assert @prim_writes_check((ob->inti)); 
    assert $writable($s, #wrTime$1^175.1, $dot($ptr(^_toto, P#ob), _toto.inti));
    // assert @reads_check_normal((ob->inti)); 
    assert $typed2($s, $dot($ptr(^_toto, P#ob), _toto.inti), ^^u8);
    assert $thread_local2($s, $dot($ptr(^_toto, P#ob), _toto.inti), ^^u8);
    // *(ob->inti) := unchecked|(*((ob->inti)), 2); 
    call $write_int($dot($ptr(^_toto, P#ob), _toto.inti), $_or(^^u8, $mem($s, $dot($ptr(^_toto, P#ob), _toto.inti)), 2));
    assume $full_stop_ext(#tok$1^183.3, $s);
    // assert @map_get(ToBm64(*((ob->inti))), 1); 
    assert $select.$map_t..^^u8.^^bool(F#ToBm64($mem($s, $dot($ptr(^_toto, P#ob), _toto.inti))), 1);
    // assume @map_get(ToBm64(*((ob->inti))), 1); 
    assume $select.$map_t..^^u8.^^bool(F#ToBm64($mem($s, $dot($ptr(^_toto, P#ob), _toto.inti))), 1);
    // assert !=(*((ob->inti)), 0); 
    assert $mem($s, $dot($ptr(^_toto, P#ob), _toto.inti)) != 0;
    // assume !=(*((ob->inti)), 0); 
    assume $mem($s, $dot($ptr(^_toto, P#ob), _toto.inti)) != 0;
    // assert ==(*((ob->inti)), 2); 
    assert $mem($s, $dot($ptr(^_toto, P#ob), _toto.inti)) == 2;
    // assume ==(*((ob->inti)), 2); 
    assume $mem($s, $dot($ptr(^_toto, P#ob), _toto.inti)) == 2;
    // assert @map_get(ToBm64(*((ob->inti))), 1); 
    assert $select.$map_t..^^u8.^^bool(F#ToBm64($mem($s, $dot($ptr(^_toto, P#ob), _toto.inti))), 1);
    // assume @map_get(ToBm64(*((ob->inti))), 1); 
    assume $select.$map_t..^^u8.^^bool(F#ToBm64($mem($s, $dot($ptr(^_toto, P#ob), _toto.inti))), 1);
    // assert forall(uint64_t i; @in_range_u8(i); ==>(&&(<(i, 64), !=(i, 1)), unchecked!(@map_get(ToBm64(*((ob->inti))), i)))); 
    assert (forall Q#i$1^188.26#tc1: int :: $in_range_u8(Q#i$1^188.26#tc1) ==> Q#i$1^188.26#tc1 < 64 && Q#i$1^188.26#tc1 != 1 ==> !$select.$map_t..^^u8.^^bool(F#ToBm64($mem($s, $dot($ptr(^_toto, P#ob), _toto.inti))), Q#i$1^188.26#tc1));
    // assume forall(uint64_t i; @in_range_u8(i); ==>(&&(<(i, 64), !=(i, 1)), unchecked!(@map_get(ToBm64(*((ob->inti))), i)))); 
    assume (forall Q#i$1^188.26#tc1: int :: $in_range_u8(Q#i$1^188.26#tc1) ==> Q#i$1^188.26#tc1 < 64 && Q#i$1^188.26#tc1 != 1 ==> !$select.$map_t..^^u8.^^bool(F#ToBm64($mem($s, $dot($ptr(^_toto, P#ob), _toto.inti))), Q#i$1^188.26#tc1));
    // return true; 
    $result := true;
    assert $position_marker();
    goto #exit;
...
axiom (forall Q#i$1^65.23#tc1: int :: $in_range_u8(Q#i$1^65.23#tc1) ==> Q#i$1^65.23#tc1 < 64 ==> !$select.$map_t..^^u8.^^bool(F#ToBm64(0), Q#i$1^65.23#tc1));

axiom (forall Q#i$1^66.23#tc1: int :: $in_range_u8(Q#i$1^66.23#tc1) ==> Q#i$1^66.23#tc1 < 64 ==> $select.$map_t..^^u8.^^bool(F#ToBm64(18446744073709551615), Q#i$1^66.23#tc1));

axiom (forall Q#i$1^67.23#tc1: int :: { F#ToBm64($_shl(^^u8, 1, Q#i$1^67.23#tc1)) } $in_range_u8(Q#i$1^67.23#tc1) ==> Q#i$1^67.23#tc1 < 64 ==> $eq.$map_t..^^u8.^^bool(F#ToBm64($_shl(^^u8, 1, Q#i$1^67.23#tc1)), F#Bm64Singleton(Q#i$1^67.23#tc1)));

axiom (forall Q#x$1^68.23#tc1: int :: { F#Bm64SpecialValue(Q#x$1^68.23#tc1) } $in_range_u8(Q#x$1^68.23#tc1) ==> (forall Q#i$1^68.64#tc1: int :: $in_range_u8(Q#i$1^68.64#tc1) ==> Q#i$1^68.64#tc1 < 64 ==> !$select.$map_t..^^u8.^^bool(F#ToBm64(Q#x$1^68.23#tc1), Q#i$1^68.64#tc1)) ==> Q#x$1^68.23#tc1 == 0);

axiom (forall Q#x$1^69.23#tc1: int :: { F#Bm64SpecialValue(Q#x$1^69.23#tc1) } $in_range_u8(Q#x$1^69.23#tc1) ==> (forall Q#i$1^69.64#tc1: int :: $in_range_u8(Q#i$1^69.64#tc1) ==> Q#i$1^69.64#tc1 < 64 ==> $select.$map_t..^^u8.^^bool(F#ToBm64(Q#x$1^69.23#tc1), Q#i$1^69.64#tc1)) ==> Q#x$1^69.23#tc1 == 18446744073709551615);

axiom (forall Q#x$1^70.23#tc1: int, Q#y$1^70.23#tc1: int :: { F#ToBm64($_and(^^u8, Q#x$1^70.23#tc1, Q#y$1^70.23#tc1)) } $in_range_u8(Q#x$1^70.23#tc1) && $in_range_u8(Q#y$1^70.23#tc1) ==> $eq.$map_t..^^u8.^^bool(F#ToBm64($_and(^^u8, Q#x$1^70.23#tc1, Q#y$1^70.23#tc1)), F#Bm64Intersect(F#ToBm64(Q#x$1^70.23#tc1), F#ToBm64(Q#y$1^70.23#tc1))));

axiom (forall Q#x$1^71.23#tc1: int, Q#y$1^71.23#tc1: int :: { F#ToBm64($_or(^^u8, Q#x$1^71.23#tc1, Q#y$1^71.23#tc1)) } $in_range_u8(Q#x$1^71.23#tc1) && $in_range_u8(Q#y$1^71.23#tc1) ==> $eq.$map_t..^^u8.^^bool(F#ToBm64($_or(^^u8, Q#x$1^71.23#tc1, Q#y$1^71.23#tc1)), F#Bm64Union(F#ToBm64(Q#x$1^71.23#tc1), F#ToBm64(Q#y$1^71.23#tc1))));

axiom (forall Q#x$1^72.23#tc1: int, Q#y$1^72.23#tc1: int :: { F#ToBm64($_xor(^^u8, Q#x$1^72.23#tc1, Q#y$1^72.23#tc1)) } $in_range_u8(Q#x$1^72.23#tc1) && $in_range_u8(Q#y$1^72.23#tc1) ==> $eq.$map_t..^^u8.^^bool(F#ToBm64($_xor(^^u8, Q#x$1^72.23#tc1, Q#y$1^72.23#tc1)), F#Bm64SymmetricDiff(F#ToBm64(Q#x$1^72.23#tc1), F#ToBm64(Q#y$1^72.23#tc1))));

axiom (forall Q#x$1^73.23#tc1: int, Q#i$1^73.23#tc1: int :: $in_range_u8(Q#x$1^73.23#tc1) && $in_range_u8(Q#i$1^73.23#tc1) ==> (Q#i$1^73.23#tc1 < 64 ==> $select.$map_t..^^u8.^^bool(F#ToBm64(Q#x$1^73.23#tc1), Q#i$1^73.23#tc1)) == ($_and(^^u8, Q#x$1^73.23#tc1, $_shl(^^u8, 1, Q#i$1^73.23#tc1)) != 0));

axiom (forall Q#i$1^75.23#tc1: int :: $in_range_u8(Q#i$1^75.23#tc1) ==> Q#i$1^75.23#tc1 < 32 ==> !$select.$map_t..^^u8.^^bool(F#ToBm32(0), Q#i$1^75.23#tc1));

axiom (forall Q#i$1^76.23#tc1: int :: $in_range_u8(Q#i$1^76.23#tc1) ==> Q#i$1^76.23#tc1 < 32 ==> $select.$map_t..^^u8.^^bool(F#ToBm32(4294967295), Q#i$1^76.23#tc1));

axiom (forall Q#i$1^77.23#tc1: int :: { F#ToBm32($_shl(^^u4, 1, Q#i$1^77.23#tc1)) } $in_range_u8(Q#i$1^77.23#tc1) ==> Q#i$1^77.23#tc1 < 32 ==> $eq.$map_t..^^u8.^^bool(F#ToBm32($_shl(^^u4, 1, Q#i$1^77.23#tc1)), F#Bm32Singleton(Q#i$1^77.23#tc1)));

axiom (forall Q#x$1^78.23#tc2: int :: { F#Bm32SpecialValue(Q#x$1^78.23#tc2) } $in_range_u4(Q#x$1^78.23#tc2) ==> (forall Q#i$1^78.64#tc2: int :: $in_range_u4(Q#i$1^78.64#tc2) ==> Q#i$1^78.64#tc2 < 32 ==> !$select.$map_t..^^u8.^^bool(F#ToBm32(Q#x$1^78.23#tc2), Q#i$1^78.64#tc2)) ==> Q#x$1^78.23#tc2 == 0);

axiom (forall Q#x$1^79.23#tc2: int :: { F#Bm32SpecialValue(Q#x$1^79.23#tc2) } $in_range_u4(Q#x$1^79.23#tc2) ==> (forall Q#i$1^79.64#tc2: int :: $in_range_u4(Q#i$1^79.64#tc2) ==> Q#i$1^79.64#tc2 < 32 ==> $select.$map_t..^^u8.^^bool(F#ToBm32(Q#x$1^79.23#tc2), Q#i$1^79.64#tc2)) ==> Q#x$1^79.23#tc2 == 4294967295);

axiom (forall Q#x$1^80.23#tc2: int, Q#y$1^80.23#tc2: int :: { F#ToBm32($_and(^^u4, Q#x$1^80.23#tc2, Q#y$1^80.23#tc2)) } $in_range_u4(Q#x$1^80.23#tc2) && $in_range_u4(Q#y$1^80.23#tc2) ==> $eq.$map_t..^^u8.^^bool(F#ToBm32($_and(^^u4, Q#x$1^80.23#tc2, Q#y$1^80.23#tc2)), F#Bm32Intersect(F#ToBm32(Q#x$1^80.23#tc2), F#ToBm32(Q#y$1^80.23#tc2))));

axiom (forall Q#x$1^81.23#tc2: int, Q#y$1^81.23#tc2: int :: { F#ToBm32($_or(^^u4, Q#x$1^81.23#tc2, Q#y$1^81.23#tc2)) } $in_range_u4(Q#x$1^81.23#tc2) && $in_range_u4(Q#y$1^81.23#tc2) ==> $eq.$map_t..^^u8.^^bool(F#ToBm32($_or(^^u4, Q#x$1^81.23#tc2, Q#y$1^81.23#tc2)), F#Bm32Union(F#ToBm32(Q#x$1^81.23#tc2), F#ToBm32(Q#y$1^81.23#tc2))));

axiom (forall Q#x$1^82.23#tc2: int, Q#y$1^82.23#tc2: int :: { F#ToBm32($_xor(^^u4, Q#x$1^82.23#tc2, Q#y$1^82.23#tc2)) } $in_range_u4(Q#x$1^82.23#tc2) && $in_range_u4(Q#y$1^82.23#tc2) ==> $eq.$map_t..^^u8.^^bool(F#ToBm32($_xor(^^u4, Q#x$1^82.23#tc2, Q#y$1^82.23#tc2)), F#Bm32SymmetricDiff(F#ToBm32(Q#x$1^82.23#tc2), F#ToBm32(Q#y$1^82.23#tc2))));

axiom (forall Q#x$1^83.23#tc2: int, Q#i$1^83.23#tc1: int :: $in_range_u4(Q#x$1^83.23#tc2) && $in_range_u8(Q#i$1^83.23#tc1) ==> (Q#i$1^83.23#tc1 < 32 ==> $select.$map_t..^^u8.^^bool(F#ToBm32(Q#x$1^83.23#tc2), Q#i$1^83.23#tc1)) == ($_and(^^u4, Q#x$1^83.23#tc2, $_shl(^^u4, 1, Q#i$1^83.23#tc1)) != 0));
...
axiom (forall Q#i$1^52.38#tc1: int, #l1: $map_t..^^u8.^^bool, #l0: $map_t..^^u8.^^bool :: { $select.$map_t..^^u8.^^bool(F#lambda#8(#l1, #l0), Q#i$1^52.38#tc1) } $in_range_u8(Q#i$1^52.38#tc1) ==> $select.$map_t..^^u8.^^bool(F#lambda#8(#l1, #l0), Q#i$1^52.38#tc1) == (Q#i$1^52.38#tc1 < 32 ==> !($select.$map_t..^^u8.^^bool(#l0, Q#i$1^52.38#tc1) == $select.$map_t..^^u8.^^bool(#l1, Q#i$1^52.38#tc1))));
...
axiom (forall Q#i$1^47.38#tc1: int, #l1: $map_t..^^u8.^^bool, #l0: $map_t..^^u8.^^bool :: { $select.$map_t..^^u8.^^bool(F#lambda#7(#l1, #l0), Q#i$1^47.38#tc1) } $in_range_u8(Q#i$1^47.38#tc1) ==> $select.$map_t..^^u8.^^bool(F#lambda#7(#l1, #l0), Q#i$1^47.38#tc1) == (Q#i$1^47.38#tc1 < 64 ==> !($select.$map_t..^^u8.^^bool(#l0, Q#i$1^47.38#tc1) == $select.$map_t..^^u8.^^bool(#l1, Q#i$1^47.38#tc1))));
...
axiom (forall Q#i$1^42.38#tc1: int, #l1: $map_t..^^u8.^^bool, #l0: $map_t..^^u8.^^bool :: { $select.$map_t..^^u8.^^bool(F#lambda#6(#l1, #l0), Q#i$1^42.38#tc1) } $in_range_u8(Q#i$1^42.38#tc1) ==> $select.$map_t..^^u8.^^bool(F#lambda#6(#l1, #l0), Q#i$1^42.38#tc1) == (Q#i$1^42.38#tc1 < 32 ==> $select.$map_t..^^u8.^^bool(#l0, Q#i$1^42.38#tc1) && $select.$map_t..^^u8.^^bool(#l1, Q#i$1^42.38#tc1)));
...
axiom (forall Q#i$1^37.38#tc1: int, #l1: $map_t..^^u8.^^bool, #l0: $map_t..^^u8.^^bool :: { $select.$map_t..^^u8.^^bool(F#lambda#5(#l1, #l0), Q#i$1^37.38#tc1) } $in_range_u8(Q#i$1^37.38#tc1) ==> $select.$map_t..^^u8.^^bool(F#lambda#5(#l1, #l0), Q#i$1^37.38#tc1) == (Q#i$1^37.38#tc1 < 64 ==> $select.$map_t..^^u8.^^bool(#l0, Q#i$1^37.38#tc1) && $select.$map_t..^^u8.^^bool(#l1, Q#i$1^37.38#tc1)));
...
axiom (forall Q#i$1^32.38#tc1: int, #l1: $map_t..^^u8.^^bool, #l0: $map_t..^^u8.^^bool :: { $select.$map_t..^^u8.^^bool(F#lambda#4(#l1, #l0), Q#i$1^32.38#tc1) } $in_range_u8(Q#i$1^32.38#tc1) ==> $select.$map_t..^^u8.^^bool(F#lambda#4(#l1, #l0), Q#i$1^32.38#tc1) == (Q#i$1^32.38#tc1 < 32 ==> $select.$map_t..^^u8.^^bool(#l0, Q#i$1^32.38#tc1) || $select.$map_t..^^u8.^^bool(#l1, Q#i$1^32.38#tc1)));
...
axiom (forall Q#i$1^27.38#tc1: int, #l1: $map_t..^^u8.^^bool, #l0: $map_t..^^u8.^^bool :: { $select.$map_t..^^u8.^^bool(F#lambda#3(#l1, #l0), Q#i$1^27.38#tc1) } $in_range_u8(Q#i$1^27.38#tc1) ==> $select.$map_t..^^u8.^^bool(F#lambda#3(#l1, #l0), Q#i$1^27.38#tc1) == (Q#i$1^27.38#tc1 < 64 ==> $select.$map_t..^^u8.^^bool(#l0, Q#i$1^27.38#tc1) || $select.$map_t..^^u8.^^bool(#l1, Q#i$1^27.38#tc1)));
...
axiom (forall Q#j$1^22.111#tc1: int, #l0: int :: { $select.$map_t..^^u8.^^bool(F#lambda#2(#l0), Q#j$1^22.111#tc1) } $in_range_u8(Q#j$1^22.111#tc1) && $in_range_u8(#l0) ==> $select.$map_t..^^u8.^^bool(F#lambda#2(#l0), Q#j$1^22.111#tc1) == (Q#j$1^22.111#tc1 == #l0));
...
axiom (forall Q#j$1^18.111#tc1: int, #l0: int :: { $select.$map_t..^^u8.^^bool(F#lambda#1(#l0), Q#j$1^18.111#tc1) } $in_range_u8(Q#j$1^18.111#tc1) && $in_range_u8(#l0) ==> $select.$map_t..^^u8.^^bool(F#lambda#1(#l0), Q#j$1^18.111#tc1) == (Q#j$1^18.111#tc1 == #l0));
...
axiom (forall M: $map_t..^^u8.^^bool, p: int, v: bool :: $in_range_t(^^u8, p) ==> $select.$map_t..^^u8.^^bool($store.$map_t..^^u8.^^bool(M, p, v), p) == v);

axiom (forall M: $map_t..^^u8.^^bool, p: int, v: bool, q: int :: p != q ==> $select.$map_t..^^u8.^^bool($store.$map_t..^^u8.^^bool(M, q, v), p) == $select.$map_t..^^u8.^^bool(M, p));

axiom (forall M1: $map_t..^^u8.^^bool, M2: $map_t..^^u8.^^bool :: { $eq.$map_t..^^u8.^^bool(M1, M2) } (forall p: int :: $in_range_t(^^u8, p) ==> $select.$map_t..^^u8.^^bool(M1, p) == $select.$map_t..^^u8.^^bool(M2, p)) ==> $eq.$map_t..^^u8.^^bool(M1, M2));
...
axiom (forall r1: $record, r2: $record, f: $field, R: $ctype :: { $rec_base_eq($rec_fetch(r1, f), $rec_fetch(r2, f)), $is_record_field(R, f, $map_t(^^u8, ^^bool)) } $eq.$map_t..^^u8.^^bool($int_to_map_t..^^u8.^^bool($rec_fetch(r1, f)), $int_to_map_t..^^u8.^^bool($rec_fetch(r2, f))) ==> $rec_base_eq($rec_fetch(r1, f), $rec_fetch(r2, f)));
...
const unique #file^C?3A?5Cdev?5Cfelt?5Cvcc?5CTest?5Ctestsuite?5Cexamples?5CBitMap.c: $token;

axiom $file_name_is(1, #file^C?3A?5Cdev?5Cfelt?5Cvcc?5CTest?5Ctestsuite?5Cexamples?5CBitMap.c);
