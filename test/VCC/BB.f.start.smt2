;Analysis time until now:     1s
(set-option :print-success false)
(set-info :smt-lib-version 2.0)
(set-option :AUTO_CONFIG false)
(set-option :pp.bv_literals false)
(set-option :MODEL.V2 true)
(set-option :smt.PHASE_SELECTION 0)
(set-option :smt.RESTART_STRATEGY 0)
(set-option :smt.RESTART_FACTOR |1.5|)
(set-option :smt.ARITH.RANDOM_INITIAL_VALUE true)
(set-option :smt.CASE_SPLIT 3)
(set-option :smt.DELAY_UNITS true)
(set-option :NNF.SK_HACK true)
(set-option :smt.MBQI false)
(set-option :smt.QI.EAGER_THRESHOLD 100)
(set-option :TYPE_CHECK true)
(set-option :smt.BV.REFLECT true)
(set-option :TIMEOUT 0)
(set-logic  UFNIA)
;(set-option :produce-unsat-cores true)
; done setting options
;Sorts
(declare-sort $@Map@@$ptr@To@Bool@@ 0)
(declare-sort $ctype 0)
(declare-sort $field 0)
(declare-sort $kind 0)
(declare-sort $memory_t 0)
(declare-sort $ptr 0)
(declare-sort $ptrclub 0)
(declare-sort $ptrset 0)
(declare-sort $pure_function 0)
(declare-sort $state 0)
(declare-sort $status 0)
(declare-sort $statusmap_t 0)
(declare-sort $token 0)
(declare-sort $type_state 0)
(declare-sort $typemap_t 0)
(declare-sort $version 0)
(declare-sort $vol_version 0)

;Functions
(declare-fun $@bf_71 ( ) Bool)
(declare-fun $@bf_72 ( ) Bool)
(declare-fun $@bf_73 ( ) Bool)
(declare-fun $@bf_74 ( ) Bool)
(declare-fun $@bf_75 ( ) Bool)
(declare-fun $@bf_76 ( ) Bool)
(declare-fun $@bf_77 ( ) Bool)
(declare-fun $@bf_78 ( ) Bool)
(declare-fun $@bf_79 ( ) Bool)
(declare-fun $@bf_80 ( ) Bool)
(declare-fun $@bf_81 ( ) Bool)
(declare-fun $@bf_82 ( ) Bool)
(declare-fun $@distTp1 ( ) $ctype)
(declare-fun $@distTp2 ( ) $ctype)
(declare-fun $@distTp3 ( ) $ctype)
(declare-fun $@file^C?3A?5Cdev?5Cfelt?5Cvcc?5CTest?5Ctestsuite?5Cexamples?5CLockFreeIncr.c ( ) $token)
(declare-fun $@loc._this_ ( ) $token)
(declare-fun $@tok$1^13.1 ( ) $token)
(declare-fun $@wrTime$1^13.1 ( ) Int)
(declare-fun $arch_ptr_size ( ) Int)
(declare-fun $arch_spec_ptr_start ( ) Int)
(declare-fun $array ( $ctype Int ) $ctype)
(declare-fun $current_frame_level ( ) Int)
(declare-fun $current_pure_post_level ( ) Int)
(declare-fun $current_timestamp ( $state ) Int)
(declare-fun $dot ( $ptr $field ) $ptr)
(declare-fun $extent ( $state $ptr ) $ptrset)
(declare-fun $fetch_from_vv ( $vol_version $ptr ) Int)
(declare-fun $field_parent_type ( $field ) $ctype)
(declare-fun $frame_level ( $pure_function ) Int)
(declare-fun $full_extent ( $ptr ) $ptrset)
(declare-fun $idx ( $ptr Int $ctype ) $ptr)
(declare-fun $index_within ( $ptr $ptr ) Int)
(declare-fun $int_to_ptr ( Int ) $ptr)
(declare-fun $kind_array ( ) $kind)
(declare-fun $kind_composite ( ) $kind)
(declare-fun $kind_of ( $ctype ) $kind)
(declare-fun $kind_primitive ( ) $kind)
(declare-fun $kind_thread ( ) $kind)
(declare-fun $me ( ) $ptr)
(declare-fun $memory ( $state ) $memory_t)
(declare-fun $memory_allocator_type ( ) $ctype)
(declare-fun $no_claim ( ) $ptr)
(declare-fun $null ( ) $ptr)
(declare-fun $owner ( $state $ptr ) $ptr)
(declare-fun $owns ( $state $ptr ) $ptrset)
(declare-fun $ptr ( $ctype Int ) $ptr)
(declare-fun $ptr_level ( $ctype ) Int)
(declare-fun $ptr_to ( $ctype ) $ctype)
(declare-fun $ptr_to_int ( $ptr ) Int)
(declare-fun $ptrclub.members ( $ptrclub ) $@Map@@$ptr@To@Bool@@)
(declare-fun $pure_post_level ( $pure_function ) Int)
(declare-fun $read_version ( $state $ptr ) $version)
(declare-fun $read_vol_version ( $state $ptr ) $vol_version)
(declare-fun $ref ( $ptr ) Int)
(declare-fun $ref_cnt ( $state $ptr ) Int)
(declare-fun $s ( ) $state)
(declare-fun $s@0 ( ) $state)
(declare-fun $select.mem ( $memory_t $ptr ) Int)
(declare-fun $select.sm ( $statusmap_t $ptr ) $status)
(declare-fun $select.tm ( $typemap_t $ptr ) $type_state)
(declare-fun $select_range.mem ( $memory_t $ptr Int ) Int)
(declare-fun $set_difference ( $ptrset $ptrset ) $ptrset)
(declare-fun $set_empty ( ) $ptrset)
(declare-fun $set_singleton ( $ptr ) $ptrset)
(declare-fun $set_union ( $ptrset $ptrset ) $ptrset)
(declare-fun $span ( $ptr ) $ptrset)
(declare-fun $statusmap ( $state ) $statusmap_t)
(declare-fun $timestamp ( $state $ptr ) Int)
(declare-fun $ts_active_option ( $type_state ) $field)
(declare-fun $ts_emb ( $type_state ) $ptr)
(declare-fun $ts_path ( $type_state ) $field)
(declare-fun $typ ( $ptr ) $ctype)
(declare-fun $typemap ( $state ) $typemap_t)
(declare-fun ^$@thread_id ( ) $ctype)
(declare-fun ^^claim ( ) $ctype)
(declare-fun ^^i4 ( ) $ctype)
(declare-fun ^^root_emb ( ) $ctype)
(declare-fun ^^void ( ) $ctype)
(declare-fun ^A ( ) $ctype)
(declare-fun A.$owns ( ) $field)
(declare-fun A.x ( ) $field)
(declare-fun call0formal@@l@0 ( ) $ptr)
(declare-fun P@_this_ ( ) Int)

;Predicates
(declare-fun $can_use_all_frame_axioms ( $state ) Bool)
(declare-fun $closed ( $state $ptr ) Bool)
(declare-fun $extent_mutable ( $state $ptr ) Bool)
(declare-fun $file_name_is ( Int $token ) Bool)
(declare-fun $forall_inv2_when_closed ( $state $state ) Bool)
(declare-fun $full_stop ( $state ) Bool)
(declare-fun $function_entry ( $state ) Bool)
(declare-fun $good_for_pre_can_unwrap ( $state ) Bool)
(declare-fun $good_state ( $state ) Bool)
(declare-fun $good_state_ext ( $token $state ) Bool)
(declare-fun $has_volatile_owns_set ( $ctype ) Bool)
(declare-fun $in_range_phys_ptr ( Int ) Bool)
(declare-fun $in_range_spec_ptr ( Int ) Bool)
(declare-fun $in_wrapped_domain ( $state $ptr ) Bool)
(declare-fun $in_writes_at ( Int $ptr ) Bool)
(declare-fun $inv2 ( $state $state $ptr $ctype ) Bool)
(declare-fun $is ( $ptr $ctype ) Bool)
(declare-fun $is_base_field ( $field ) Bool)
(declare-fun $is_claimable ( $ctype ) Bool)
(declare-fun $is_composite ( $ctype ) Bool)
(declare-fun $is_math_type ( $ctype ) Bool)
(declare-fun $is_non_primitive ( $ctype ) Bool)
(declare-fun $is_primitive_non_volatile_field ( $field ) Bool)
(declare-fun $is_primitive_volatile_field ( $field ) Bool)
(declare-fun $is_stuttering_check ( ) Bool)
(declare-fun $is_threadtype ( $ctype ) Bool)
(declare-fun $is_unwrap_check ( ) Bool)
(declare-fun $local_value_is ( $state $token $token Int $ctype ) Bool)
(declare-fun $local_value_is_ptr ( $state $token $token $ptr $ctype ) Bool)
(declare-fun $mutable ( $state $ptr ) Bool)
(declare-fun $nonvolatile_spans_the_same ( $state $state $ptr $ctype ) Bool)
(declare-fun $position_marker ( ) Bool)
(declare-fun $seclbl.bot ( ) Bool)
(declare-fun $seclbl.top ( ) Bool)
(declare-fun $set_eq ( $ptrset $ptrset ) Bool)
(declare-fun $set_in ( $ptr $ptrset ) Bool)
(declare-fun $spans_the_same ( $state $state $ptr $ctype ) Bool)
(declare-fun $state_nonvolatile_spans_the_same ( $state $state $ptr $ctype ) Bool)
(declare-fun $state_spans_the_same ( $state $state $ptr $ctype ) Bool)
(declare-fun $stuttering_pre ( $state $ptr ) Bool)
(declare-fun $thread_local ( $state $ptr ) Bool)
(declare-fun $ts_is_volatile ( $type_state ) Bool)
(declare-fun $typed ( $state $ptr ) Bool)
(declare-fun $valid_claim ( $state $ptr ) Bool)
(declare-fun MapRead$$@Map@@$ptr@To@Bool@@$ ( $@Map@@$ptr@To@Bool@@ $ptr ) Bool)

;Axioms
   (assert (! (not (= $@distTp1 $memory_allocator_type)) :named ax_0))
   (assert (! (not (= $@distTp2 $@distTp1)) :named ax_1))
   (assert (! (not (= $@distTp2 $@distTp1)) :named ax_2))
   (assert (! (not (= $@distTp2 $memory_allocator_type)) :named ax_3))
   (assert (! (not (= $@distTp3 $@distTp1)) :named ax_4))
   (assert (! (not (= $@distTp3 $@distTp1)) :named ax_5))
   (assert (! (not (= $@distTp3 $@distTp2)) :named ax_6))
   (assert (! (not (= $@distTp3 $@distTp2)) :named ax_7))
   (assert (! (not (= $@distTp3 $memory_allocator_type)) :named ax_8))
   (assert (! (not (= $@loc._this_ $@file^C?3A?5Cdev?5Cfelt?5Cvcc?5CTest?5Ctestsuite?5Cexamples?5CLockFreeIncr.c)) :named ax_9))
   (assert (! (not (= $@loc._this_ $@file^C?3A?5Cdev?5Cfelt?5Cvcc?5CTest?5Ctestsuite?5Cexamples?5CLockFreeIncr.c)) :named ax_10))
   (assert (! (not (= $@loc._this_ $@tok$1^13.1)) :named ax_11))
   (assert (! (not (= $@tok$1^13.1 $@file^C?3A?5Cdev?5Cfelt?5Cvcc?5CTest?5Ctestsuite?5Cexamples?5CLockFreeIncr.c)) :named ax_12))
   (assert (! (not (= $@tok$1^13.1 $@file^C?3A?5Cdev?5Cfelt?5Cvcc?5CTest?5Ctestsuite?5Cexamples?5CLockFreeIncr.c)) :named ax_13))
   (assert (! (not (= $@tok$1^13.1 $@loc._this_)) :named ax_14))
   (assert (! (not (= $kind_array $kind_thread)) :named ax_15))
   (assert (! (not (= $kind_composite $kind_array)) :named ax_16))
   (assert (! (not (= $kind_composite $kind_array)) :named ax_17))
   (assert (! (not (= $kind_composite $kind_primitive)) :named ax_18))
   (assert (! (not (= $kind_composite $kind_thread)) :named ax_19))
   (assert (! (not (= $kind_primitive $kind_array)) :named ax_20))
   (assert (! (not (= $kind_primitive $kind_array)) :named ax_21))
   (assert (! (not (= $kind_primitive $kind_composite)) :named ax_22))
   (assert (! (not (= $kind_primitive $kind_thread)) :named ax_23))
   (assert (! (not (= $kind_thread $kind_array)) :named ax_24))
   (assert (! (not (= $kind_thread $kind_composite)) :named ax_25))
   (assert (! (not (= $kind_thread $kind_primitive)) :named ax_26))
   (assert (! (not (= $memory_allocator_type $@distTp1)) :named ax_27))
   (assert (! (not (= $memory_allocator_type $@distTp2)) :named ax_28))
   (assert (! (not (= $memory_allocator_type $@distTp3)) :named ax_29))
   (assert (! (not (= $memory_allocator_type ^$@thread_id)) :named ax_30))
   (assert (! (not (= $memory_allocator_type ^A)) :named ax_31))
   (assert (! (not (= ^$@thread_id $@distTp1)) :named ax_32))
   (assert (! (not (= ^$@thread_id $@distTp1)) :named ax_33))
   (assert (! (not (= ^$@thread_id $@distTp2)) :named ax_34))
   (assert (! (not (= ^$@thread_id $@distTp2)) :named ax_35))
   (assert (! (not (= ^$@thread_id $@distTp3)) :named ax_36))
   (assert (! (not (= ^$@thread_id $@distTp3)) :named ax_37))
   (assert (! (not (= ^$@thread_id $memory_allocator_type)) :named ax_38))
   (assert (! (not (= ^$@thread_id ^A)) :named ax_39))
   (assert (! (not (= ^^claim $@distTp1)) :named ax_40))
   (assert (! (not (= ^^claim $@distTp1)) :named ax_41))
   (assert (! (not (= ^^claim $@distTp2)) :named ax_42))
   (assert (! (not (= ^^claim $@distTp2)) :named ax_43))
   (assert (! (not (= ^^claim $@distTp3)) :named ax_44))
   (assert (! (not (= ^^claim $@distTp3)) :named ax_45))
   (assert (! (not (= ^^claim $memory_allocator_type)) :named ax_46))
   (assert (! (not (= ^^claim $memory_allocator_type)) :named ax_47))
   (assert (! (not (= ^^claim ^$@thread_id)) :named ax_48))
   (assert (! (not (= ^^claim ^$@thread_id)) :named ax_49))
   (assert (! (not (= ^^claim ^^root_emb)) :named ax_50))
   (assert (! (not (= ^^claim ^A)) :named ax_51))
   (assert (! (not (= ^^i4 $@distTp1)) :named ax_52))
   (assert (! (not (= ^^i4 $@distTp1)) :named ax_53))
   (assert (! (not (= ^^i4 $@distTp2)) :named ax_54))
   (assert (! (not (= ^^i4 $@distTp2)) :named ax_55))
   (assert (! (not (= ^^i4 $@distTp3)) :named ax_56))
   (assert (! (not (= ^^i4 $@distTp3)) :named ax_57))
   (assert (! (not (= ^^i4 $memory_allocator_type)) :named ax_58))
   (assert (! (not (= ^^i4 $memory_allocator_type)) :named ax_59))
   (assert (! (not (= ^^i4 ^$@thread_id)) :named ax_60))
   (assert (! (not (= ^^i4 ^$@thread_id)) :named ax_61))
   (assert (! (not (= ^^i4 ^^claim)) :named ax_62))
   (assert (! (not (= ^^i4 ^^claim)) :named ax_63))
   (assert (! (not (= ^^i4 ^^root_emb)) :named ax_64))
   (assert (! (not (= ^^i4 ^^void)) :named ax_65))
   (assert (! (not (= ^^i4 ^A)) :named ax_66))
   (assert (! (not (= ^^root_emb $@distTp1)) :named ax_67))
   (assert (! (not (= ^^root_emb $@distTp1)) :named ax_68))
   (assert (! (not (= ^^root_emb $@distTp2)) :named ax_69))
   (assert (! (not (= ^^root_emb $@distTp2)) :named ax_70))
   (assert (! (not (= ^^root_emb $@distTp3)) :named ax_71))
   (assert (! (not (= ^^root_emb $@distTp3)) :named ax_72))
   (assert (! (not (= ^^root_emb $memory_allocator_type)) :named ax_73))
   (assert (! (not (= ^^root_emb $memory_allocator_type)) :named ax_74))
   (assert (! (not (= ^^root_emb ^$@thread_id)) :named ax_75))
   (assert (! (not (= ^^root_emb ^$@thread_id)) :named ax_76))
   (assert (! (not (= ^^root_emb ^^claim)) :named ax_77))
   (assert (! (not (= ^^root_emb ^^i4)) :named ax_78))
   (assert (! (not (= ^^root_emb ^A)) :named ax_79))
   (assert (! (not (= ^^void $@distTp1)) :named ax_80))
   (assert (! (not (= ^^void $@distTp1)) :named ax_81))
   (assert (! (not (= ^^void $@distTp2)) :named ax_82))
   (assert (! (not (= ^^void $@distTp2)) :named ax_83))
   (assert (! (not (= ^^void $@distTp3)) :named ax_84))
   (assert (! (not (= ^^void $@distTp3)) :named ax_85))
   (assert (! (not (= ^^void $memory_allocator_type)) :named ax_86))
   (assert (! (not (= ^^void $memory_allocator_type)) :named ax_87))
   (assert (! (not (= ^^void ^$@thread_id)) :named ax_88))
   (assert (! (not (= ^^void ^$@thread_id)) :named ax_89))
   (assert (! (not (= ^^void ^^claim)) :named ax_90))
   (assert (! (not (= ^^void ^^claim)) :named ax_91))
   (assert (! (not (= ^^void ^^i4)) :named ax_92))
   (assert (! (not (= ^^void ^^root_emb)) :named ax_93))
   (assert (! (not (= ^^void ^^root_emb)) :named ax_94))
   (assert (! (not (= ^^void ^A)) :named ax_95))
   (assert (! (not (= ^A $@distTp1)) :named ax_96))
   (assert (! (not (= ^A $@distTp1)) :named ax_97))
   (assert (! (not (= ^A $@distTp2)) :named ax_98))
   (assert (! (not (= ^A $@distTp2)) :named ax_99))
   (assert (! (not (= ^A $@distTp3)) :named ax_100))
   (assert (! (not (= ^A $@distTp3)) :named ax_101))
   (assert (! (not (= ^A $memory_allocator_type)) :named ax_102))
   (assert (! (not (= ^A ^$@thread_id)) :named ax_103))
   (assert (! (not (= ^A ^^claim)) :named ax_104))
   (assert (! (not (= ^A ^^i4)) :named ax_105))
   (assert (! (not (= ^A ^^root_emb)) :named ax_106))
   (assert (! (not (= ^A ^^void)) :named ax_107))
   (assert (! (not (= A.$owns A.x)) :named ax_108))
   (assert (! (not (= A.x A.$owns)) :named ax_109))

;Program
(assert (not
(let ((anon6_assertion (and
    (=> $is_unwrap_check ($set_eq ($owns $s@0 ($ptr ^A P@_this_)) $set_empty))
)))
(let ((anon6 (=> (and true
    $is_unwrap_check
    ($can_use_all_frame_axioms $s)
    ($closed $s ($ptr ^A P@_this_))
    ($full_stop $s)
    ($function_entry $s)
    ($good_for_pre_can_unwrap $s)
    ($good_state_ext $@tok$1^13.1 $s)
    ($good_state_ext $@tok$1^13.1 $s@0)
    ($inv2 $s $s ($ptr ^A P@_this_) ($typ ($ptr ^A P@_this_)))
    ($is ($ptr ^A P@_this_) ($typ ($ptr ^A P@_this_)))
    ($is_non_primitive ($typ ($ptr ^A P@_this_)))
    ($local_value_is $s $@tok$1^13.1 $@loc._this_ ($ptr_to_int ($ptr ^A P@_this_)) ($ptr_to ^A))
    ($local_value_is_ptr $s $@tok$1^13.1 $@loc._this_ ($ptr ^A P@_this_) ($ptr_to ^A))
    ($spans_the_same $s $s@0 call0formal@@l@0 ($typ call0formal@@l@0))
    ($typed $s ($ptr ^A P@_this_))
    (= $@wrTime$1^13.1 ($current_timestamp $s))
    (= ($owner $s ($ptr ^A P@_this_)) $me)
    (= call0formal@@l@0 ($ptr ^A P@_this_))
    (forall( (|#q_$0_$3| $ptr))(!(=> ($closed $s |#q_$0_$3|) ($inv2 $s $s |#q_$0_$3| ($typ |#q_$0_$3|)))  :pattern ( ($select.sm ($statusmap $s) |#q_$0_$3|))))
    (not $is_stuttering_check)
    (not (= ($kind_of ($typ ($ptr ^A P@_this_))) $kind_primitive))
    (or (not ($is_claimable ($typ ($ptr ^A P@_this_)))) (= ($ref_cnt $s ($ptr ^A P@_this_)) 0))
  )
    anon6_assertion
)))
(let ((PreconditionGeneratedEntry (=> (and true
  )
    anon6
)))
(let (($start (=> (and true
    $position_marker
    ($file_name_is 1 $@file^C?3A?5Cdev?5Cfelt?5Cvcc?5CTest?5Ctestsuite?5Cexamples?5CLockFreeIncr.c)
    ($has_volatile_owns_set ^^claim)
    ($is_base_field A.$owns)
    ($is_base_field A.x)
    ($is_composite ^A)
    ($is_math_type ^$@thread_id)
    ($is_primitive_non_volatile_field A.$owns)
    ($is_primitive_volatile_field A.x)
    (= $@distTp1 ($ptr_to ^A))
    (= $@distTp3 ($ptr_to ^^i4))
    (= $arch_ptr_size 8)
    (= $no_claim ($ptr ^^claim 0))
    (= $null ($ptr ^^void 0))
    (= $seclbl.bot false)
    (= $seclbl.top true)
    (= ($field_parent_type A.$owns) ^A)
    (= ($field_parent_type A.x) ^A)
    (= ($has_volatile_owns_set ^A) false)
    (= ($is_claimable ^A) false)
    (= ($ptr_level $memory_allocator_type) 0)
    (= ($ptr_level ^A) 0)
    (= (and (not $is_stuttering_check) (not $is_unwrap_check)) (and (not $is_stuttering_check) (not $is_unwrap_check)))
    (= true $@bf_71)
    (= true $@bf_71)
    (= true $@bf_72)
    (= true $@bf_72)
    (= true $@bf_74)
    (= true $@bf_74)
    (= true $@bf_75)
    (= true $@bf_75)
    (= true $@bf_79)
    (= true $@bf_79)
    (= true $@bf_80)
    (= true $@bf_80)
    (= true $@bf_81)
    (= true $@bf_81)
    (forall( (|#l_$2| $ptr) (|#p_$14| $ptr))(= ($set_in |#p_$14| ($span |#l_$2|)) ($set_in |#p_$14| ($span |#l_$2|))))
    (forall( (|#l_$3| $ptr) (|#p_$16| $ptr))(= ($set_in |#p_$16| ($full_extent |#l_$3|)) ($set_in |#p_$16| ($full_extent |#l_$3|))))
    (forall( (|#o| $ptr) (|#f| $ptr) (|S_$19| $state))(= (and (= ($kind_of ($typ |#f|)) $kind_primitive) (= ($ts_emb ($select.tm ($typemap |S_$19|) |#f|)) |#o|)) (and (= ($kind_of ($typ |#f|)) $kind_primitive) (= ($ts_emb ($select.tm ($typemap |S_$19|) |#f|)) |#o|))))
    (forall( (|#p_$0| $ptr) (|S_$0| $state))(= ($ts_emb ($select.tm ($typemap |S_$0|) |#p_$0|)) ($ts_emb ($select.tm ($typemap |S_$0|) |#p_$0|))))
    (forall( (|#p_$1| $ptr) (|S_$1| $state))(= ($ts_path ($select.tm ($typemap |S_$1|) |#p_$1|)) ($ts_path ($select.tm ($typemap |S_$1|) |#p_$1|))))
    (forall( (|#p_$10| $ptr) (|S_$25| $state))(= (and ($typed |S_$25| |#p_$10|) (not ($ts_is_volatile ($select.tm ($typemap |S_$25|) |#p_$10|)))) (and ($typed |S_$25| |#p_$10|) (not ($ts_is_volatile ($select.tm ($typemap |S_$25|) |#p_$10|))))))
    (forall( (|#p_$11| $ptr) (|#l| $ptr) (|S_$31| $state) (|S0| $state))(= (and (and (and (and (and ($typed |S0| |#p_$11|) (and (and (and (and ($closed |S_$31| |#p_$11|) (= ($owner |S_$31| |#p_$11|) $me)) (and ($is |#p_$11| ($typ |#p_$11|)) ($typed |S_$31| |#p_$11|))) (not (= ($kind_of ($typ |#p_$11|)) $kind_primitive))) ($is_non_primitive ($typ |#p_$11|)))) (= ($timestamp |S_$31| |#p_$11|) ($current_timestamp |S_$31|))) ($is_non_primitive ($typ |#p_$11|))) ($set_in |#p_$11| ($owns |S0| |#l|))) (and (= ($owner |S0| |#p_$11|) |#l|) ($closed |S0| |#p_$11|))) (and (and (and (and (and ($typed |S0| |#p_$11|) (and (and (and (and ($closed |S_$31| |#p_$11|) (= ($owner |S_$31| |#p_$11|) $me)) (and ($is |#p_$11| ($typ |#p_$11|)) ($typed |S_$31| |#p_$11|))) (not (= ($kind_of ($typ |#p_$11|)) $kind_primitive))) ($is_non_primitive ($typ |#p_$11|)))) (= ($timestamp |S_$31| |#p_$11|) ($current_timestamp |S_$31|))) ($is_non_primitive ($typ |#p_$11|))) ($set_in |#p_$11| ($owns |S0| |#l|))) (and (= ($owner |S0| |#p_$11|) |#l|) ($closed |S0| |#p_$11|)))))
    (forall( (|#p_$12| $ptr) (|#l_$0| $ptr) (|S_$32| $state) (|S0_$0| $state))(= (and (and (and (and (and (and ($typed |S0_$0| |#p_$12|) (and (and (and (and ($closed |S_$32| |#p_$12|) (= ($owner |S_$32| |#p_$12|) $me)) (and ($is |#p_$12| ($typ |#p_$12|)) ($typed |S_$32| |#p_$12|))) (not (= ($kind_of ($typ |#p_$12|)) $kind_primitive))) ($is_non_primitive ($typ |#p_$12|)))) (= ($timestamp |S_$32| |#p_$12|) ($current_timestamp |S_$32|))) ($is_non_primitive ($typ |#p_$12|))) ($set_in |#p_$12| ($owns |S0_$0| |#l_$0|))) (and (= ($owner |S0_$0| |#p_$12|) |#l_$0|) ($closed |S0_$0| |#p_$12|))) (=> ($is_claimable ($typ |#p_$12|)) (= ($ref_cnt |S0_$0| |#p_$12|) ($ref_cnt |S_$32| |#p_$12|)))) (and (and (and (and (and (and ($typed |S0_$0| |#p_$12|) (and (and (and (and ($closed |S_$32| |#p_$12|) (= ($owner |S_$32| |#p_$12|) $me)) (and ($is |#p_$12| ($typ |#p_$12|)) ($typed |S_$32| |#p_$12|))) (not (= ($kind_of ($typ |#p_$12|)) $kind_primitive))) ($is_non_primitive ($typ |#p_$12|)))) (= ($timestamp |S_$32| |#p_$12|) ($current_timestamp |S_$32|))) ($is_non_primitive ($typ |#p_$12|))) ($set_in |#p_$12| ($owns |S0_$0| |#l_$0|))) (and (= ($owner |S0_$0| |#p_$12|) |#l_$0|) ($closed |S0_$0| |#p_$12|))) (=> ($is_claimable ($typ |#p_$12|)) (= ($ref_cnt |S0_$0| |#p_$12|) ($ref_cnt |S_$32| |#p_$12|))))))
    (forall( (|#p_$13| $ptr) (|#l_$1| $ptr) (|S_$33| $state))(= ($set_in |#p_$13| ($owns |S_$33| |#l_$1|)) ($set_in |#p_$13| ($owns |S_$33| |#l_$1|))))
    (forall( (|#p_$15| $ptr))(= ($full_extent |#p_$15|) ($full_extent |#p_$15|)))
    (forall( (|#p2_$0| $ptr) (|#p1_$0| $ptr))(= ($set_in |#p1_$0| ($full_extent |#p2_$0|)) ($set_in |#p1_$0| ($full_extent |#p2_$0|))))
    (forall( (|#p2| $ptr) (|#p1| $ptr) (|S_$22| $state))(= ($set_in |#p1| ($extent |S_$22| |#p2|)) ($set_in |#p1| ($extent |S_$22| |#p2|))))
    (forall( (|#r_$0| Int))(!(= ($in_range_spec_ptr |#r_$0|) (or (= 0 |#r_$0|) (> |#r_$0| $arch_spec_ptr_start)))  :pattern ( ($in_range_spec_ptr |#r_$0|))))
    (forall( (|#r| Int))(!(= ($in_range_phys_ptr |#r|) (and (<= 0 |#r|) (<= |#r| $arch_spec_ptr_start)))  :pattern ( ($in_range_phys_ptr |#r|))))
    (forall( (|#s1_$5| $state) (|#s2_$3| $state) (|#p_$18| $ptr) (|#t_$8| $ctype))(!(= ($spans_the_same |#s1_$5| |#s2_$3| |#p_$18| |#t_$8|) (and (and (and (= ($read_version |#s1_$5| |#p_$18|) ($read_version |#s2_$3| |#p_$18|)) (= ($owns |#s1_$5| |#p_$18|) ($owns |#s2_$3| |#p_$18|))) (= ($select.tm ($typemap |#s1_$5|) |#p_$18|) ($select.tm ($typemap |#s2_$3|) |#p_$18|))) ($state_spans_the_same |#s1_$5| |#s2_$3| |#p_$18| |#t_$8|)))  :weight  0 :pattern ( ($spans_the_same |#s1_$5| |#s2_$3| |#p_$18| |#t_$8|))))
    (forall( (|#s1_$6| $state) (|#s2_$4| $state) (|#p_$19| $ptr) (|#t_$9| $ctype))(!(= ($nonvolatile_spans_the_same |#s1_$6| |#s2_$4| |#p_$19| |#t_$9|) (and (and (= ($read_version |#s1_$6| |#p_$19|) ($read_version |#s2_$4| |#p_$19|)) (= ($select.tm ($typemap |#s1_$6|) |#p_$19|) ($select.tm ($typemap |#s2_$4|) |#p_$19|))) ($state_nonvolatile_spans_the_same |#s1_$6| |#s2_$4| |#p_$19| |#t_$9|)))  :weight  0 :pattern ( ($nonvolatile_spans_the_same |#s1_$6| |#s2_$4| |#p_$19| |#t_$9|))))
    (forall( (|#s1_$7| $state) (|#s2_$5| $state) (|#p_$20| $ptr))(!(= ($inv2 |#s1_$7| |#s2_$5| |#p_$20| ^A) (and (and ($typed |#s2_$5| |#p_$20|) ($set_eq ($owns |#s2_$5| |#p_$20|) $set_empty)) (or (= ($select.mem ($memory |#s1_$7|) ($dot |#p_$20| A.x)) ($select.mem ($memory |#s2_$5|) ($dot |#p_$20| A.x))) (= (+ ($select.mem ($memory |#s1_$7|) ($dot |#p_$20| A.x)) 1) ($select.mem ($memory |#s2_$5|) ($dot |#p_$20| A.x))))))  :pattern ( ($inv2 |#s1_$7| |#s2_$5| |#p_$20| ^A))))
    (forall( (|#t_$0| $ctype) (|#p_$2| $ptr) (|S_$12| $state))(= (and (and (and (and ($closed |S_$12| |#p_$2|) (= ($owner |S_$12| |#p_$2|) $me)) (and ($is |#p_$2| |#t_$0|) ($typed |S_$12| |#p_$2|))) (not (= ($kind_of |#t_$0|) $kind_primitive))) ($is_non_primitive |#t_$0|)) (and (and (and (and ($closed |S_$12| |#p_$2|) (= ($owner |S_$12| |#p_$2|) $me)) (and ($is |#p_$2| |#t_$0|) ($typed |S_$12| |#p_$2|))) (not (= ($kind_of |#t_$0|) $kind_primitive))) ($is_non_primitive |#t_$0|))))
    (forall( (|#t_$1| $ctype) (|#p_$3| $ptr) (|S_$16| $state))(= (and ($is |#p_$3| |#t_$1|) ($typed |S_$16| |#p_$3|)) (and ($is |#p_$3| |#t_$1|) ($typed |S_$16| |#p_$3|))))
    (forall( (|#t_$2| $ctype) (|#p_$4| $ptr) (|S_$17| $state))(= (and (and ($is |#p_$4| |#t_$2|) ($typed |S_$17| |#p_$4|)) (=> (and ($is |#p_$4| |#t_$2|) ($typed |S_$17| |#p_$4|)) ($in_range_phys_ptr ($ref |#p_$4|)))) (and (and ($is |#p_$4| |#t_$2|) ($typed |S_$17| |#p_$4|)) (=> (and ($is |#p_$4| |#t_$2|) ($typed |S_$17| |#p_$4|)) ($in_range_phys_ptr ($ref |#p_$4|))))))
    (forall( (|#t_$3| $ctype) (|#p_$5| $ptr) (|S_$18| $state))(= (and (and ($is |#p_$5| |#t_$3|) ($typed |S_$18| |#p_$5|)) (=> (and ($is |#p_$5| |#t_$3|) ($typed |S_$18| |#p_$5|)) ($in_range_spec_ptr ($ref |#p_$5|)))) (and (and ($is |#p_$5| |#t_$3|) ($typed |S_$18| |#p_$5|)) (=> (and ($is |#p_$5| |#t_$3|) ($typed |S_$18| |#p_$5|)) ($in_range_spec_ptr ($ref |#p_$5|))))))
    (forall( (|#t_$4| $ctype) (|#p_$6| $ptr) (|S_$21| $state))(= (and ($is |#p_$6| |#t_$4|) ($thread_local |S_$21| |#p_$6|)) (and ($is |#p_$6| |#t_$4|) ($thread_local |S_$21| |#p_$6|))))
    (forall( (|#t_$5| $ctype) (|#b| Int))(!(= ($typ ($ptr |#t_$5| |#b|)) |#t_$5|)  :weight  0))
    (forall( (|#t_$6| $ctype) (|#b_$0| Int))(!(= ($ref ($ptr |#t_$6| |#b_$0|)) |#b_$0|)  :weight  0))
    (forall( (|#t| $ctype) (|#p| $ptr))(= ($ptr |#t| ($ref |#p|)) ($ptr |#t| ($ref |#p|))))
    (forall( (|c_$1| $ptrclub) (|p_$33| $ptr))(= (MapRead$$@Map@@$ptr@To@Bool@@$ ($ptrclub.members |c_$1|) |p_$33|) (MapRead$$@Map@@$ptr@To@Bool@@$ ($ptrclub.members |c_$1|) |p_$33|)))
    (forall( (|e_$0| $ptr) (|S_$39| $ptrset))(= ($set_difference |S_$39| ($set_singleton |e_$0|)) ($set_difference |S_$39| ($set_singleton |e_$0|))))
    (forall( (|e| $ptr) (|S_$38| $ptrset))(= ($set_union |S_$38| ($set_singleton |e|)) ($set_union |S_$38| ($set_singleton |e|))))
    (forall( (|emb| $ptr) (|p_$31| $ptr) (|S_$40| $state))(= (and (and (= ($ts_emb ($select.tm ($typemap |S_$40|) |p_$31|)) |emb|) (not ($ts_is_volatile ($select.tm ($typemap |S_$40|) |p_$31|)))) ($typed |S_$40| |p_$31|)) (and (and (= ($ts_emb ($select.tm ($typemap |S_$40|) |p_$31|)) |emb|) (not ($ts_is_volatile ($select.tm ($typemap |S_$40|) |p_$31|)))) ($typed |S_$40| |p_$31|))))
    (forall( (|f_$1| $field) (|p_$25| $ptr) (|S_$26| $state))(= ($fetch_from_vv ($read_vol_version |S_$26| |p_$25|) ($dot |p_$25| |f_$1|)) ($fetch_from_vv ($read_vol_version |S_$26| |p_$25|) ($dot |p_$25| |f_$1|))))
    (forall( (|f_$2| $field) (|u_$0| $ptr) (|S_$36| $state))(= (= ($ts_active_option ($select.tm ($typemap |S_$36|) |u_$0|)) |f_$2|) (= ($ts_active_option ($select.tm ($typemap |S_$36|) |u_$0|)) |f_$2|)))
    (forall( (|f_$3| $pure_function))(= (< ($frame_level |f_$3|) $current_frame_level) (< ($frame_level |f_$3|) $current_frame_level)))
    (forall( (|f_$4| $pure_function))(= (< ($pure_post_level |f_$4|) $current_pure_post_level) (< ($pure_post_level |f_$4|) $current_pure_post_level)))
    (forall( (|isvolatile| Bool) (|tp| $ctype) (|f_$0| $field) (|p_$8| $ptr) (|S_$2| $state))(= (and (and (and (and ($is ($dot |p_$8| |f_$0|) |tp|) ($typed |S_$2| ($dot |p_$8| |f_$0|))) (= ($ts_emb ($select.tm ($typemap |S_$2|) ($dot |p_$8| |f_$0|))) |p_$8|)) (= ($ts_path ($select.tm ($typemap |S_$2|) ($dot |p_$8| |f_$0|))) |f_$0|)) (= ($ts_is_volatile ($select.tm ($typemap |S_$2|) ($dot |p_$8| |f_$0|))) |isvolatile|)) (and (and (and (and ($is ($dot |p_$8| |f_$0|) |tp|) ($typed |S_$2| ($dot |p_$8| |f_$0|))) (= ($ts_emb ($select.tm ($typemap |S_$2|) ($dot |p_$8| |f_$0|))) |p_$8|)) (= ($ts_path ($select.tm ($typemap |S_$2|) ($dot |p_$8| |f_$0|))) |f_$0|)) (= ($ts_is_volatile ($select.tm ($typemap |S_$2|) ($dot |p_$8| |f_$0|))) |isvolatile|))))
    (forall( (|l2_$0| Bool) (|l1_$0| Bool))(= (and |l1_$0| |l2_$0|) (and |l1_$0| |l2_$0|)))
    (forall( (|l2_$1| Bool) (|l1_$1| Bool))(= (or |l1_$1| |l2_$1|) (or |l1_$1| |l2_$1|)))
    (forall( (|l2| Bool) (|l1| Bool))(= (=> |l1| |l2|) (=> |l1| |l2|)))
    (forall( (|max| Int) (|val| Int) (|min| Int))(= (and (<= |min| |val|) (<= |val| |max|)) (and (<= |min| |val|) (<= |val| |max|))))
    (forall( (|owner| $ptr) (|p_$18| $ptr) (|S_$11| $state))(= (and (= ($owner |S_$11| |p_$18|) |owner|) ($closed |S_$11| |p_$18|)) (and (= ($owner |S_$11| |p_$18|) |owner|) ($closed |S_$11| |p_$18|))))
    (forall( (|owns| $ptrset) (|o| $ptr) (|S_$34| $state))(= ($set_difference |owns| ($owns |S_$34| |o|)) ($set_difference |owns| ($owns |S_$34| |o|))))
    (forall( (|p_$0| $ptr))(= (= ($kind_of ($typ |p_$0|)) $kind_composite) (= ($kind_of ($typ |p_$0|)) $kind_composite)))
    (forall( (|p_$1| $ptr))(= ($is_non_primitive ($typ |p_$1|)) ($is_non_primitive ($typ |p_$1|))))
    (forall( (|p_$10| $ptr) (|S_$4| $state))(= ($ts_is_volatile ($select.tm ($typemap |S_$4|) |p_$10|)) ($ts_is_volatile ($select.tm ($typemap |S_$4|) |p_$10|))))
    (forall( (|p_$11| $ptr) (|S_$5| $state))(= (= ($ts_emb ($select.tm ($typemap |S_$5|) |p_$11|)) ($ptr ^^root_emb ($ref |p_$11|))) (= ($ts_emb ($select.tm ($typemap |S_$5|) |p_$11|)) ($ptr ^^root_emb ($ref |p_$11|)))))
    (forall( (|p_$12| $ptr) (|M2| $state) (|M1| $state))(= (and (< ($current_timestamp |M1|) ($timestamp |M2| |p_$12|)) (<= ($timestamp |M2| |p_$12|) ($current_timestamp |M2|))) (and (< ($current_timestamp |M1|) ($timestamp |M2| |p_$12|)) (<= ($timestamp |M2| |p_$12|) ($current_timestamp |M2|)))))
    (forall( (|p_$13| $ptr) (|begin_time| Int) (|S_$6| $state))(= (and (or ($in_writes_at |begin_time| |p_$13|) (>= ($timestamp |S_$6| |p_$13|) |begin_time|)) ($mutable |S_$6| |p_$13|)) (and (or ($in_writes_at |begin_time| |p_$13|) (>= ($timestamp |S_$6| |p_$13|) |begin_time|)) ($mutable |S_$6| |p_$13|))))
    (forall( (|p_$14| $ptr) (|begin_time_$0| Int) (|S_$7| $state))(= (and (or ($in_writes_at |begin_time_$0| |p_$14|) (>= ($timestamp |S_$7| |p_$14|) |begin_time_$0|)) (and (and ($typed |S_$7| |p_$14|) (= ($owner |S_$7| |p_$14|) $me)) (=> (= ($kind_of ($typ |p_$14|)) $kind_primitive) (not ($closed |S_$7| |p_$14|))))) (and (or ($in_writes_at |begin_time_$0| |p_$14|) (>= ($timestamp |S_$7| |p_$14|) |begin_time_$0|)) (and (and ($typed |S_$7| |p_$14|) (= ($owner |S_$7| |p_$14|) $me)) (=> (= ($kind_of ($typ |p_$14|)) $kind_primitive) (not ($closed |S_$7| |p_$14|)))))))
    (forall( (|p_$15| $ptr) (|S_$8| $state))(= ($select.sm ($statusmap |S_$8|) |p_$15|) ($select.sm ($statusmap |S_$8|) |p_$15|)))
    (forall( (|p_$16| $ptr) (|S_$9| $state))(= ($select.tm ($typemap |S_$9|) |p_$16|) ($select.tm ($typemap |S_$9|) |p_$16|)))
    (forall( (|p_$17| $ptr) (|S_$10| $state))(= (not (= ($kind_of ($typ ($owner |S_$10| |p_$17|))) $kind_thread)) (not (= ($kind_of ($typ ($owner |S_$10| |p_$17|))) $kind_thread))))
    (forall( (|p_$19| $ptr) (|S_$13| $state))(= (or (not (= ($owner |S_$13| |p_$19|) $me)) (and (= ($kind_of ($typ |p_$19|)) $kind_primitive) ($closed |S_$13| |p_$19|))) (or (not (= ($owner |S_$13| |p_$19|) $me)) (and (= ($kind_of ($typ |p_$19|)) $kind_primitive) ($closed |S_$13| |p_$19|)))))
    (forall( (|p_$2| $ptr) (|s_$0| $state))(= ($select.mem ($memory |s_$0|) |p_$2|) ($select.mem ($memory |s_$0|) |p_$2|)))
    (forall( (|p_$20| $ptr) (|S_$14| $state))(= (and ($typed |S_$14| |p_$20|) (= ($owner |S_$14| |p_$20|) $me)) (and ($typed |S_$14| |p_$20|) (= ($owner |S_$14| |p_$20|) $me))))
    (forall( (|p_$21| $ptr) (|S_$15| $state))(= (and (and ($typed |S_$15| |p_$21|) (= ($owner |S_$15| |p_$21|) $me)) (=> (= ($kind_of ($typ |p_$21|)) $kind_primitive) (not ($closed |S_$15| |p_$21|)))) (and (and ($typed |S_$15| |p_$21|) (= ($owner |S_$15| |p_$21|) $me)) (=> (= ($kind_of ($typ |p_$21|)) $kind_primitive) (not ($closed |S_$15| |p_$21|))))))
    (forall( (|p_$23| $ptr) (|S_$20| $state))(= (and (not (= ($kind_of ($typ |p_$23|)) $kind_primitive)) (or (= ($owner |S_$20| |p_$23|) $me) ($in_wrapped_domain |S_$20| |p_$23|))) (and (not (= ($kind_of ($typ |p_$23|)) $kind_primitive)) (or (= ($owner |S_$20| |p_$23|) $me) ($in_wrapped_domain |S_$20| |p_$23|)))))
    (forall( (|p_$24| $ptr) (|S2| $state) (|S1| $state))(= ($inv2 |S1| |S2| |p_$24| ($typ |p_$24|)) ($inv2 |S1| |S2| |p_$24| ($typ |p_$24|))))
    (forall( (|p_$26| $ptr) (|S_$27| $state))(= (and (and ($closed |S_$27| |p_$26|) ($inv2 |S_$27| |S_$27| |p_$26| ($typ |p_$26|))) ($stuttering_pre |S_$27| |p_$26|)) (and (and ($closed |S_$27| |p_$26|) ($inv2 |S_$27| |S_$27| |p_$26| ($typ |p_$26|))) ($stuttering_pre |S_$27| |p_$26|))))
    (forall( (|p_$27| $ptr) (|S_$28| $state))(= (and (and (and (and (and (and (and (and ($closed |S_$28| |p_$27|) (= ($owner |S_$28| |p_$27|) $me)) (and ($is |p_$27| ($typ |p_$27|)) ($typed |S_$28| |p_$27|))) (not (= ($kind_of ($typ |p_$27|)) $kind_primitive))) ($is_non_primitive ($typ |p_$27|))) (or (not ($is_claimable ($typ |p_$27|))) (= ($ref_cnt |S_$28| |p_$27|) 0))) ($inv2 |S_$28| |S_$28| |p_$27| ($typ |p_$27|))) (forall( (|#q_$0_$1| $ptr))(!(=> ($closed |S_$28| |#q_$0_$1|) ($inv2 |S_$28| |S_$28| |#q_$0_$1| ($typ |#q_$0_$1|)))  :pattern ( ($select.sm ($statusmap |S_$28|) |#q_$0_$1|))))) ($good_for_pre_can_unwrap |S_$28|)) (and (and (and (and (and (and (and (and ($closed |S_$28| |p_$27|) (= ($owner |S_$28| |p_$27|) $me)) (and ($is |p_$27| ($typ |p_$27|)) ($typed |S_$28| |p_$27|))) (not (= ($kind_of ($typ |p_$27|)) $kind_primitive))) ($is_non_primitive ($typ |p_$27|))) (or (not ($is_claimable ($typ |p_$27|))) (= ($ref_cnt |S_$28| |p_$27|) 0))) ($inv2 |S_$28| |S_$28| |p_$27| ($typ |p_$27|))) (forall( (|#q_$0| $ptr))(!(=> ($closed |S_$28| |#q_$0|) ($inv2 |S_$28| |S_$28| |#q_$0| ($typ |#q_$0|)))  :pattern ( ($select.sm ($statusmap |S_$28|) |#q_$0|))))) ($good_for_pre_can_unwrap |S_$28|))))
    (forall( (|p_$28| $ptr) (|S_$29| $state))(= (= ($timestamp |S_$29| |p_$28|) ($current_timestamp |S_$29|)) (= ($timestamp |S_$29| |p_$28|) ($current_timestamp |S_$29|))))
    (forall( (|p_$29| $ptr) (|S_$30| $state))(= (and (= ($timestamp |S_$30| |p_$29|) ($current_timestamp |S_$30|)) ($mutable |S_$30| |p_$29|)) (and (= ($timestamp |S_$30| |p_$29|) ($current_timestamp |S_$30|)) ($mutable |S_$30| |p_$29|))))
    (forall( (|p_$30| $ptr) (|S_$37| $state))(= (and (and ($extent_mutable |S_$37| |p_$30|) (= ($ts_emb ($select.tm ($typemap |S_$37|) |p_$30|)) ($ptr ^^root_emb ($ref |p_$30|)))) (= ($timestamp |S_$37| |p_$30|) ($current_timestamp |S_$37|))) (and (and ($extent_mutable |S_$37| |p_$30|) (= ($ts_emb ($select.tm ($typemap |S_$37|) |p_$30|)) ($ptr ^^root_emb ($ref |p_$30|)))) (= ($timestamp |S_$37| |p_$30|) ($current_timestamp |S_$37|)))))
    (forall( (|p_$4| $ptr) (|s2| $state) (|s1| $state))(= (= ($select.mem ($memory |s1|) |p_$4|) ($select.mem ($memory |s2|) |p_$4|)) (= ($select.mem ($memory |s1|) |p_$4|) ($select.mem ($memory |s2|) |p_$4|))))
    (forall( (|p_$5| $ptr) (|s2_$0| $state) (|s1_$0| $state))(= (= ($select.sm ($statusmap |s1_$0|) |p_$5|) ($select.sm ($statusmap |s2_$0|) |p_$5|)) (= ($select.sm ($statusmap |s1_$0|) |p_$5|) ($select.sm ($statusmap |s2_$0|) |p_$5|))))
    (forall( (|p_$6| $ptr) (|s2_$1| $state) (|s1_$1| $state))(= (= ($select.tm ($typemap |s1_$1|) |p_$6|) ($select.tm ($typemap |s2_$1|) |p_$6|)) (= ($select.tm ($typemap |s1_$1|) |p_$6|) ($select.tm ($typemap |s2_$1|) |p_$6|))))
    (forall( (|p_$9| $ptr) (|S_$3| $state))(= (= ($ts_emb ($select.tm ($typemap |S_$3|) |p_$9|)) ($ptr ^^root_emb ($ref |p_$9|))) (= ($ts_emb ($select.tm ($typemap |S_$3|) |p_$9|)) ($ptr ^^root_emb ($ref |p_$9|)))))
    (forall( (|p| $ptr))(= ($is_threadtype ($typ |p|)) ($is_threadtype ($typ |p|))))
    (forall( (|p2_$0| $ptr) (|p1_$0| $ptr))(= (not (= ($ref |p1_$0|) ($ref |p2_$0|))) (not (= ($ref |p1_$0|) ($ref |p2_$0|)))))
    (forall( (|p2| $ptr) (|p1| $ptr))(= (= ($ref |p1|) ($ref |p2|)) (= ($ref |p1|) ($ref |p2|))))
    (forall( (|S_$23| $state) (|t_$5| $token))(= (and ($good_state_ext |t_$5| |S_$23|) ($full_stop |S_$23|)) (and ($good_state_ext |t_$5| |S_$23|) ($full_stop |S_$23|))))
    (forall( (|S_$24| $state))(= (forall( (|#p_$9_$0| $ptr) (|#q_$1| $ptr))(!(=> (and (and ($good_state |S_$24|) ($set_in |#p_$9_$0| ($owns |S_$24| |#q_$1|))) ($closed |S_$24| |#q_$1|)) (and ($closed |S_$24| |#p_$9_$0|) (not (= ($ref |#p_$9_$0|) 0))))  :pattern ( ($set_in |#p_$9_$0| ($owns |S_$24| |#q_$1|))))) (forall( (|#p_$9| $ptr) (|#q| $ptr))(!(=> (and (and ($good_state |S_$24|) ($set_in |#p_$9| ($owns |S_$24| |#q|))) ($closed |S_$24| |#q|)) (and ($closed |S_$24| |#p_$9|) (not (= ($ref |#p_$9|) 0))))  :pattern ( ($set_in |#p_$9| ($owns |S_$24| |#q|)))))))
    (forall( (|S_$42| $state) (|p_$34| $ptr))(!(= ($mutable |S_$42| |p_$34|) (and (and ($typed |S_$42| |p_$34|) (= ($owner |S_$42| |p_$34|) $me)) (not ($closed |S_$42| |p_$34|))))  :weight  0 :pattern ( ($mutable |S_$42| |p_$34|))))
    (forall( (|S_$44| $state) (|p_$36| $ptr))(!(= ($thread_local |S_$44| |p_$36|) (and ($typed |S_$44| |p_$36|) (or (and (and (= ($kind_of ($typ |p_$36|)) $kind_primitive) (or (not ($ts_is_volatile ($select.tm ($typemap |S_$44|) |p_$36|))) (not ($closed |S_$44| ($ts_emb ($select.tm ($typemap |S_$44|) |p_$36|)))))) (and (not (= ($kind_of ($typ ($ts_emb ($select.tm ($typemap |S_$44|) |p_$36|)))) $kind_primitive)) (or (= ($owner |S_$44| ($ts_emb ($select.tm ($typemap |S_$44|) |p_$36|))) $me) ($in_wrapped_domain |S_$44| ($ts_emb ($select.tm ($typemap |S_$44|) |p_$36|)))))) (and (not (= ($kind_of ($typ |p_$36|)) $kind_primitive)) (or (= ($owner |S_$44| |p_$36|) $me) ($in_wrapped_domain |S_$44| |p_$36|))))))  :pattern ( ($thread_local |S_$44| |p_$36|))))
    (forall( (|s| $state))(= |s| |s|))
    (forall( (|s2_$2| $state) (|s1_$2| $state))(= (and (= ($typemap |s1_$2|) ($typemap |s2_$2|)) (= ($statusmap |s1_$2|) ($statusmap |s2_$2|))) (and (= ($typemap |s1_$2|) ($typemap |s2_$2|)) (= ($statusmap |s1_$2|) ($statusmap |s2_$2|)))))
    (forall( (|subject_$0| $field) (|approver_$0| $field) (|this_$0| $ptr) (|S2_$1| $state) (|S1_$1| $state))(= (or (or (or (= ($select.mem ($memory |S1_$1|) ($dot |this_$0| |subject_$0|)) ($select.mem ($memory |S2_$1|) ($dot |this_$0| |subject_$0|))) (= ($ref ($int_to_ptr ($select.mem ($memory |S1_$1|) ($dot |this_$0| |approver_$0|)))) 0)) (and (not ($is_threadtype ($typ ($int_to_ptr ($select.mem ($memory |S1_$1|) ($dot |this_$0| |approver_$0|)))))) ($inv2 |S1_$1| |S2_$1| ($int_to_ptr ($select.mem ($memory |S1_$1|) ($dot |this_$0| |approver_$0|))) ($typ ($int_to_ptr ($select.mem ($memory |S1_$1|) ($dot |this_$0| |approver_$0|))))))) (and ($is_threadtype ($typ ($int_to_ptr ($select.mem ($memory |S1_$1|) ($dot |this_$0| |approver_$0|))))) (not (= ($read_vol_version |S1_$1| |this_$0|) ($read_vol_version |S2_$1| |this_$0|))))) (or (or (or (= ($select.mem ($memory |S1_$1|) ($dot |this_$0| |subject_$0|)) ($select.mem ($memory |S2_$1|) ($dot |this_$0| |subject_$0|))) (= ($ref ($int_to_ptr ($select.mem ($memory |S1_$1|) ($dot |this_$0| |approver_$0|)))) 0)) (and (not ($is_threadtype ($typ ($int_to_ptr ($select.mem ($memory |S1_$1|) ($dot |this_$0| |approver_$0|)))))) ($inv2 |S1_$1| |S2_$1| ($int_to_ptr ($select.mem ($memory |S1_$1|) ($dot |this_$0| |approver_$0|))) ($typ ($int_to_ptr ($select.mem ($memory |S1_$1|) ($dot |this_$0| |approver_$0|))))))) (and ($is_threadtype ($typ ($int_to_ptr ($select.mem ($memory |S1_$1|) ($dot |this_$0| |approver_$0|))))) (not (= ($read_vol_version |S1_$1| |this_$0|) ($read_vol_version |S2_$1| |this_$0|)))))))
    (forall( (|subject_$1| $field) (|this_$1| $ptr) (|S2_$2| $state) (|S1_$2| $state))(= (or (or (or (= ($select.mem ($memory |S1_$2|) ($dot |this_$1| |subject_$1|)) ($select.mem ($memory |S2_$2|) ($dot |this_$1| |subject_$1|))) (= ($ref ($owner |S1_$2| |this_$1|)) 0)) (and (not ($is_threadtype ($typ ($owner |S1_$2| |this_$1|)))) ($inv2 |S1_$2| |S2_$2| ($owner |S1_$2| |this_$1|) ($typ ($owner |S1_$2| |this_$1|))))) (and ($is_threadtype ($typ ($owner |S1_$2| |this_$1|))) (not (= ($read_vol_version |S1_$2| |this_$1|) ($read_vol_version |S2_$2| |this_$1|))))) (or (or (or (= ($select.mem ($memory |S1_$2|) ($dot |this_$1| |subject_$1|)) ($select.mem ($memory |S2_$2|) ($dot |this_$1| |subject_$1|))) (= ($ref ($owner |S1_$2| |this_$1|)) 0)) (and (not ($is_threadtype ($typ ($owner |S1_$2| |this_$1|)))) ($inv2 |S1_$2| |S2_$2| ($owner |S1_$2| |this_$1|) ($typ ($owner |S1_$2| |this_$1|))))) (and ($is_threadtype ($typ ($owner |S1_$2| |this_$1|))) (not (= ($read_vol_version |S1_$2| |this_$1|) ($read_vol_version |S2_$2| |this_$1|)))))))
    (forall( (|subject| $field) (|approver| $ptr) (|this| $ptr) (|S2_$0| $state) (|S1_$0| $state))(= (or (or (or (= ($select.mem ($memory |S1_$0|) ($dot |this| |subject|)) ($select.mem ($memory |S2_$0|) ($dot |this| |subject|))) (= ($ref |approver|) 0)) (and (not ($is_threadtype ($typ |approver|))) ($inv2 |S1_$0| |S2_$0| |approver| ($typ |approver|)))) (and ($is_threadtype ($typ |approver|)) (not (= ($read_vol_version |S1_$0| |this|) ($read_vol_version |S2_$0| |this|))))) (or (or (or (= ($select.mem ($memory |S1_$0|) ($dot |this| |subject|)) ($select.mem ($memory |S2_$0|) ($dot |this| |subject|))) (= ($ref |approver|) 0)) (and (not ($is_threadtype ($typ |approver|))) ($inv2 |S1_$0| |S2_$0| |approver| ($typ |approver|)))) (and ($is_threadtype ($typ |approver|)) (not (= ($read_vol_version |S1_$0| |this|) ($read_vol_version |S2_$0| |this|)))))))
    (forall( (|sz_$0| Int) (|T| $ctype) (|p_$32| $ptr))(= ($ptr ($array |T| |sz_$0|) ($ref |p_$32|)) ($ptr ($array |T| |sz_$0|) ($ref |p_$32|))))
    (forall( (|sz_$1| Int) (|T_$0| $ctype) (|arr| $ptr) (|q| $ptr))(= (and (and (<= 0 ($index_within |q| |arr|)) (<= ($index_within |q| |arr|) (- |sz_$1| 1))) (= |q| ($idx |arr| ($index_within |q| |arr|) |T_$0|))) (and (and (<= 0 ($index_within |q| |arr|)) (<= ($index_within |q| |arr|) (- |sz_$1| 1))) (= |q| ($idx |arr| ($index_within |q| |arr|) |T_$0|)))))
    (forall( (|sz_$2| Int) (|T_$1| $ctype) (|arr_$0| $ptr) (|q_$0| $ptr))(= (and (and (<= 0 ($index_within |q_$0| |arr_$0|)) (<= ($index_within |q_$0| |arr_$0|) (- |sz_$2| 1))) ($set_in |q_$0| ($full_extent ($idx |arr_$0| ($index_within |q_$0| |arr_$0|) |T_$1|)))) (and (and (<= 0 ($index_within |q_$0| |arr_$0|)) (<= ($index_within |q_$0| |arr_$0|) (- |sz_$2| 1))) ($set_in |q_$0| ($full_extent ($idx |arr_$0| ($index_within |q_$0| |arr_$0|) |T_$1|))))))
    (forall( (|sz_$3| Int) (|T_$2| $ctype) (|arr_$1| $ptr) (|q_$1| $ptr) (|S_$41| $state))(= (and (and (<= 0 ($index_within |q_$1| |arr_$1|)) (<= ($index_within |q_$1| |arr_$1|) (- |sz_$3| 1))) ($set_in |q_$1| ($extent |S_$41| ($idx |arr_$1| ($index_within |q_$1| |arr_$1|) |T_$2|)))) (and (and (<= 0 ($index_within |q_$1| |arr_$1|)) (<= ($index_within |q_$1| |arr_$1|) (- |sz_$3| 1))) ($set_in |q_$1| ($extent |S_$41| ($idx |arr_$1| ($index_within |q_$1| |arr_$1|) |T_$2|))))))
    (forall( (|sz| Int) (|p_$3| $ptr) (|s_$1| $state))(= ($select_range.mem ($memory |s_$1|) |p_$3| |sz|) ($select_range.mem ($memory |s_$1|) |p_$3| |sz|)))
    (forall( (|t_$0| $ctype))(= (= ($kind_of |t_$0|) $kind_composite) (= ($kind_of |t_$0|) $kind_composite)))
    (forall( (|t_$1| $ctype))(= (= ($kind_of |t_$1|) $kind_array) (= ($kind_of |t_$1|) $kind_array)))
    (forall( (|t_$2| $ctype))(= (not (= ($kind_of |t_$2|) $kind_primitive)) (not (= ($kind_of |t_$2|) $kind_primitive))))
    (forall( (|t_$3| $ctype) (|p_$7| $ptr) (|S| $state))(= ($ptr |t_$3| ($select.mem ($memory |S|) |p_$7|)) ($ptr |t_$3| ($select.mem ($memory |S|) |p_$7|))))
    (forall( (|t_$4| $ctype) (|f| $field))(= (and ($is_base_field |f|) (= ($field_parent_type |f|) |t_$4|)) (and ($is_base_field |f|) (= ($field_parent_type |f|) |t_$4|))))
    (forall( (|t_$8| $ctype))(!(= ($is_threadtype |t_$8|) (= ($kind_of |t_$8|) $kind_thread))  :weight  0 :pattern ( ($is_threadtype |t_$8|))))
    (forall( (|t| $ctype))(= (= ($kind_of |t|) $kind_primitive) (= ($kind_of |t|) $kind_primitive)))
    (forall( (|tok_$0| $token) (|c_$0| $ptr) (|#s2_$0| $state) (|#s1_$2| $state))(= (and (and (and (and (and (and (and (and ($good_state_ext |tok_$0| |#s1_$2|) ($full_stop |#s1_$2|)) ($good_state_ext |tok_$0| |#s2_$0|)) (forall( (|#p_$9_$3| $ptr) (|#q_$4| $ptr))(!(=> (and (and ($good_state |#s1_$2|) ($set_in |#p_$9_$3| ($owns |#s1_$2| |#q_$4|))) ($closed |#s1_$2| |#q_$4|)) (and ($closed |#s1_$2| |#p_$9_$3|) (not (= ($ref |#p_$9_$3|) 0))))  :pattern ( ($set_in |#p_$9_$3| ($owns |#s1_$2| |#q_$4|)))))) (forall( (|#p_$9_$4| $ptr) (|#q_$5| $ptr))(!(=> (and (and ($good_state |#s2_$0|) ($set_in |#p_$9_$4| ($owns |#s2_$0| |#q_$5|))) ($closed |#s2_$0| |#q_$5|)) (and ($closed |#s2_$0| |#p_$9_$4|) (not (= ($ref |#p_$9_$4|) 0))))  :pattern ( ($set_in |#p_$9_$4| ($owns |#s2_$0| |#q_$5|)))))) ($forall_inv2_when_closed |#s1_$2| |#s2_$0|)) ($valid_claim |#s1_$2| |c_$0|)) ($closed |#s2_$0| |c_$0|)) true) (and (and (and (and (and (and (and (and ($good_state_ext |tok_$0| |#s1_$2|) ($full_stop |#s1_$2|)) ($good_state_ext |tok_$0| |#s2_$0|)) (forall( (|#p_$9_$5| $ptr) (|#q_$6| $ptr))(!(=> (and (and ($good_state |#s1_$2|) ($set_in |#p_$9_$5| ($owns |#s1_$2| |#q_$6|))) ($closed |#s1_$2| |#q_$6|)) (and ($closed |#s1_$2| |#p_$9_$5|) (not (= ($ref |#p_$9_$5|) 0))))  :pattern ( ($set_in |#p_$9_$5| ($owns |#s1_$2| |#q_$6|)))))) (forall( (|#p_$9_$6| $ptr) (|#q_$7| $ptr))(!(=> (and (and ($good_state |#s2_$0|) ($set_in |#p_$9_$6| ($owns |#s2_$0| |#q_$7|))) ($closed |#s2_$0| |#q_$7|)) (and ($closed |#s2_$0| |#p_$9_$6|) (not (= ($ref |#p_$9_$6|) 0))))  :pattern ( ($set_in |#p_$9_$6| ($owns |#s2_$0| |#q_$7|)))))) ($forall_inv2_when_closed |#s1_$2| |#s2_$0|)) ($valid_claim |#s1_$2| |c_$0|)) ($closed |#s2_$0| |c_$0|)) true)))
    (forall( (|tok| $token) (|c| $ptr) (|#s1_$1| $state))(= (and (and ($good_state_ext |tok| |#s1_$1|) (forall( (|#p_$9_$1| $ptr) (|#q_$2| $ptr))(!(=> (and (and ($good_state |#s1_$1|) ($set_in |#p_$9_$1| ($owns |#s1_$1| |#q_$2|))) ($closed |#s1_$1| |#q_$2|)) (and ($closed |#s1_$1| |#p_$9_$1|) (not (= ($ref |#p_$9_$1|) 0))))  :pattern ( ($set_in |#p_$9_$1| ($owns |#s1_$1| |#q_$2|)))))) true) (and (and ($good_state_ext |tok| |#s1_$1|) (forall( (|#p_$9_$2| $ptr) (|#q_$3| $ptr))(!(=> (and (and ($good_state |#s1_$1|) ($set_in |#p_$9_$2| ($owns |#s1_$1| |#q_$3|))) ($closed |#s1_$1| |#q_$3|)) (and ($closed |#s1_$1| |#p_$9_$2|) (not (= ($ref |#p_$9_$2|) 0))))  :pattern ( ($set_in |#p_$9_$2| ($owns |#s1_$1| |#q_$3|)))))) true)))
    (forall( (|typ_$0| $ctype) (|#p_$8| $ptr) (|#s2| $state) (|#s1_$0| $state))(= (or (and (not ($closed |#s1_$0| |#p_$8|)) (not ($closed |#s2| |#p_$8|))) (and ($inv2 |#s1_$0| |#s2| |#p_$8| |typ_$0|) ($nonvolatile_spans_the_same |#s1_$0| |#s2| |#p_$8| |typ_$0|))) (or (and (not ($closed |#s1_$0| |#p_$8|)) (not ($closed |#s2| |#p_$8|))) (and ($inv2 |#s1_$0| |#s2| |#p_$8| |typ_$0|) ($nonvolatile_spans_the_same |#s1_$0| |#s2| |#p_$8| |typ_$0|)))))
    (forall( (|typ| $ctype) (|#p_$7| $ptr) (|#s1| $state))(= ($inv2 |#s1| |#s1| |#p_$7| |typ|) ($inv2 |#s1| |#s1| |#p_$7| |typ|)))
    (forall( (|u| $ptr) (|S_$35| $state))(= ($ts_active_option ($select.tm ($typemap |S_$35|) |u|)) ($ts_active_option ($select.tm ($typemap |S_$35|) |u|))))
    (forall( (|y_$0| Int) (|x_$0| Int))(= (+ |x_$0| |y_$0|) (+ |x_$0| |y_$0|)))
    (forall( (|z| Int) (|y| Int) (|x| Int))(= (+ |x| (* |y| |z|)) (+ |x| (* |y| |z|))))
    (not (= true $@bf_73))
    (not (= true $@bf_73))
    (not (= true $@bf_76))
    (not (= true $@bf_76))
    (not (= true $@bf_77))
    (not (= true $@bf_77))
    (not (= true $@bf_78))
    (not (= true $@bf_78))
    (not (= true $@bf_82))
    (not (= true $@bf_82))
  )
    PreconditionGeneratedEntry
)))
(let (($root (=> (and true
  )
    $start
)))
   $root
)))))
))

;Commands
   (check-sat)
   ;(get-unsat-core)
