...
(and (! (or %lbl%@33357  
(=> 
	(and (not $is_stuttering_check) (not $is_unwrap_check)) 
	($set_eq ($owns $s@2 ($ptr ^A |P#_this_|)) $set_empty))) :lblneg @33357) 
	(=> (=> (and (not $is_stuttering_check) (not $is_unwrap_check)) ($set_eq ($owns $s@2 ($ptr ^A |P#_this_|)) $set_empty)) 
	(and (! (or %lbl%@33374  (=> (and (not $is_stuttering_check) (not $is_unwrap_check)) (or (= ($select.mem ($memory $s) ($dot ($ptr ^A |P#_this_|) A.x)) ($select.mem ($memory $s@2) ($dot ($ptr ^A |P#_this_|) A.x))) (= (+ ($select.mem ($memory $s) ($dot ($ptr ^A |P#_this_|) A.x)) 1) ($select.mem ($memory $s@2) ($dot ($ptr ^A |P#_this_|) A.x)))))) :lblneg @33374) 
	(=> 
		(=> 
			(and (not $is_stuttering_check) (not $is_unwrap_check)) (or (= ($select.mem ($memory $s) ($dot ($ptr ^A |P#_this_|) A.x)) ($select.mem ($memory $s@2) ($dot ($ptr ^A |P#_this_|) A.x))) (= (+ ($select.mem ($memory $s) ($dot ($ptr ^A |P#_this_|) A.x)) 1) ($select.mem ($memory $s@2) ($dot ($ptr ^A |P#_this_|) A.x))))) (and (! (or %lbl%@33440  (=> $is_stuttering_check (or (= ($select.mem ($memory $s@2) ($dot ($ptr ^A |P#_this_|) A.x)) ($select.mem ($memory $s@2) ($dot ($ptr ^A |P#_this_|) A.x))) (= (+ ($select.mem ($memory $s@2) ($dot ($ptr ^A |P#_this_|) A.x)) 1) ($select.mem ($memory $s@2) ($dot ($ptr ^A |P#_this_|) A.x)))))) :lblneg @33440) (=> (=> $is_stuttering_check (or (= ($select.mem ($memory $s@2) ($dot ($ptr ^A |P#_this_|) A.x)) ($select.mem ($memory $s@2) ($dot ($ptr ^A |P#_this_|) A.x))) (= (+ ($select.mem ($memory $s@2) ($dot ($ptr ^A |P#_this_|) A.x)) 1) ($select.mem ($memory $s@2) ($dot ($ptr ^A |P#_this_|) A.x))))) (! (or %lbl%@33506  (=> $is_unwrap_check ($set_eq ($owns $s@2 ($ptr ^A |P#_this_|)) $set_empty))) :lblneg @33506))))))))))
...
(let ((anon00_Then_correct  (=> (! (and %lbl%+27074 true) :lblpos +27074) (=> (and $is_unwrap_check (not $is_stuttering_check)) (=> (and (and (and (and (and (and (and (and (and ($closed $s ($ptr ^A |P#_this_|)) (= ($owner $s ($ptr ^A |P#_this_|)) $me)) (and ($is ($ptr ^A |P#_this_|) ($typ ($ptr ^A |P#_this_|))) ($typed $s ($ptr ^A |P#_this_|)))) (not (= ($kind_of ($typ ($ptr ^A |P#_this_|))) $kind_primitive))) ($is_non_primitive ($typ ($ptr ^A |P#_this_|)))) (or (not ($is_claimable ($typ ($ptr ^A |P#_this_|)))) (= ($ref_cnt $s ($ptr ^A |P#_this_|)) 0))) ($inv2 $s $s ($ptr ^A |P#_this_|) ($typ ($ptr ^A |P#_this_|)))) (forall ((|#q@@9| T@$ptr) ) (!  (=> ($closed $s |#q@@9|) ($inv2 $s $s |#q@@9| ($typ |#q@@9|)))
 :qid |LockFree.1308:13|
 :skolemid |152|
 :pattern ( ($select.sm ($statusmap $s) |#q@@9|))
))) ($good_for_pre_can_unwrap $s)) (and (= |call0formal@#l@0| ($ptr ^A |P#_this_|)) ($spans_the_same $s $s@0 |call0formal@#l@0| ($typ |call0formal@#l@0|)))) (and (! (or %lbl%@33087 ($set_eq ($owns $s@0 ($ptr ^A |P#_this_|)) $set_empty)) :lblneg @33087) (=> ($set_eq ($owns $s@0 ($ptr ^A |P#_this_|)) $set_empty) (=> (and ($good_state_ext |#tok$1^13.1| $s@0) (= $s@2 $s@0)) |#exit_correct|))))))))
