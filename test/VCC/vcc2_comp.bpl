...
implementation find(P#l: int) returns ($result: int)
{
  ...
  anon12:
    ...
    while (true)
      ...
      free invariant (forall Q#j$1^42.31#tc3: int :: $in_range_u4(Q#j$1^42.31#tc3) ==> Q#j$1^42.31#tc3 < L#i ==> $mem($s, $dot($ptr(^Node, $select.$map_t..^^u4.$ptr_to..^Node($int_to_map_t..^^u4.ptr_to..^Node($mem($s, $dot($ptr(^List, P#l), List.seq))), Q#j$1^42.31#tc3)), Node.data)) != 0);
    {
      anon11:
