...
//    (left != null && right != null ==> left.desc !! right.desc)  // not needed
  ...
  method barrier(N: int)
    requires 0 < N;
    requires valid();
    requires before();
    modifies this, left, right;
  {
    var z := N;  // to by-pass termination checking

//A
    pc := 1;
    if(left != null) {
      while(!left.sense && 0 < z) 
        modifies left;
        ...
        invariant valid();
      {
        ...
        z := z - 1;  // to by-pass termination checking
      }  
      assume 0 < z;  // to by-pass termination checking
    }
    if(right != null) {
      while(!right.sense && 0 < z) 
        modifies right;
        invariant validDown(); // this seems necessary to get the necessary unfolding of functions
        invariant valid();
      {
